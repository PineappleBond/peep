...
const rules: KeJingRule[] = [
  ...
  },
  {
    ...
    name: "比用课",
    group: "三传",
    description: "三传取法为'比用'（知一）：四课多克，取与日干阴阳相同者。事出多端，宜择一而从。",
  ...
  },
  {
    ...
    name: "涉害课",
    group: "三传",
    description: "三传取法为'涉害'：多克且阴阳难分，比涉害深度取之。事有牵缠，历尽艰辛方成。",
    check: r => r.threeTransmissions.method.startsWith("涉害"),
  },
  {
    code: "yaoke",
    name: "遥克课",
    ...
    check: r => r.threeTransmissions.method === "蒿矢" || r.threeTransmissions.method === "弹射",
  },
  {
    code: "maoxing",
    name: "昴星课",
    ...
    check: r =>
      r.threeTransmissions.method === "昴星虎视" || r.threeTransmissions.method === "昴星冬蛇掩目",
  },
  {
    code: "biezhe",
    name: "别责课",
    group: "三传",
    description: "三传取法为'别责'：四课不备（三课），取干合/支冲。事有偏枯、不得周全。",
  ...
  },
  {
    ...
    name: "八专课",
    group: "三传",
    description: "三传取法为'八专'（独足）：干支同位（四课仅二），事难两全、独力难支。",
    check: r =>
      r.threeTransmissions.method === "八专" || r.threeTransmissions.method === "八专独足",
  },
  ...
  // ──── 盘面类（天地盘关系）────
  {
    ...
    name: "伏吟课",
    group: "盘面",
    description: "天地盘重合，十二神各居本宫。主事迟滞、伏而不动，病忧土怪，讼争田庐。",
    check: r => isFuyin(r),
  },
  {
    code: "fanyin",
    name: "返吟课",
    group: "盘面",
    description: "天地盘对冲，十二神各居冲位。主事反复、去而复来，动摇不安。",
    check: r => isFanyin(r),
  },
  ...
  // ──── 四课 / 三传特征 ────
  {
    ...
    name: "三奇课",
    group: "三传",
    description: "占日所在六甲旬的旬奇发用或入于中末传。万事和合、千殃解除。",
    check: r => {
      // 计算日干支序号
  ...
  },
  {
    ...
    name: "官爵课",
    group: "三传",
    description: "太岁、月建、本命或行年的驿马发用，同时天魁戌与太常入传。官爵印绶、得之荣华。",
    check: r => {
      const { initial, middle, final } = r.threeTransmissions;
  ...
  },
  {
    ...
    name: "三光课",
    group: "三传",
    description: "日干、日支与发用均得旺相，日上神、辰上神与发用又均乘吉将。课入三光，万事吉昌。",
    check: r => {
  ...
  },
  {
    ...
    name: "三阳课",
    group: "三传",
    description: "贵人顺行，日干寄宫与日支均乘贵前五将，发用又得季节旺相。课入三阳，官爵翱翔。",
    check: r => {
      const stem = r.fourPillars.dayStem;
      const branch = r.fourPillars.dayBranch;
      const initial = r.threeTransmissions.initial;
  ...
  },
  {
    ...
    name: "六仪课",
    group: "三传",
    description: "占日所在六甲旬的旬首地支发用、入于中传或末传。兆多喜庆、求旺相宜。",
    check: r => {
      const dayStem = r.fourPillars.dayStem;
      const dayBranch = r.fourPillars.dayBranch;
      const dayIndex = sexagenaryIndex(dayStem, dayBranch);
      ...
      const { initial, middle, final } = r.threeTransmissions;
      return initial === xunInstrument || middle === xunInstrument || final === xunInstrument;
    },
  },
  {
    code: "chongshanchuan",
    name: "冲神三传",
    group: "三传",
    description: "三传皆六冲关系（初末冲、中末冲等）。主事多冲突、反复不定。",
    check: r => {
      const { initial, middle, final } = r.threeTransmissions;
      return (
        LIU_CHONG[initial] === final || LIU_CHONG[initial] === middle || LIU_CHONG[middle] === final
      );
    },
  },
  ...
  // ──── 天将类 ────
  {
    ...
    check: r => {
      ...
      const targets = [dayStemLodging, dayBranch, r.fate?.mingGong ?? -1, r.fate?.xingNian ?? -1];
  ...
  },
  {
    ...
    name: "龙德课",
    group: "天将",
    description: "太岁乘天乙贵人发用，月将又入于三传。君恩及下、万姓欢忻。",
    check: r => {
  ...
  },
  {
    ...
    name: "时泰课",
    group: "天将",
    description: "初末传乘青龙、六合相对，太岁或月建入传并兼作日财或日德。皇恩欲拜、灾患潜消。",
    check: r => {
      ...
      const yearQualifies = yearInTrans && (isDayWealth(yearBranch) || yearBranch === dayVirtue);
  ...
  },
  {
    ...
    name: "铸印课",
    group: "天将",
    description: "天魁戌与太乙巳同入三传；戌为印、巳为炉。顽金铸篆、藉火功全。",
    check: r => {
      const { initial, middle, final } = r.threeTransmissions;
      // 戌(10) 与 巳(5) 同入三传
      const transmissions = [initial, middle, final];
  ...
  },
  {
    ...
    name: "斫轮课",
    group: "天将",
    description: "初传卯加临地盘申（庚）或酉（辛）发用；木就金斫、革故鼎新。",
    check: r => {
      const initial = r.threeTransmissions.initial;
      // 初传为卯(3)
  ...
  // ──── 特殊类 ────
  {
    ...
    name: "天祸课",
    group: "特殊",
    description: "四立日，今日干支临昨日干支，或昨日干支临今日干支。以新易旧、天有灾祸。",
    check: r => {
      ...
      const todayOnYesterday =
        todayStemUpper === yesterdayStemLodge && todayBranchUpper === yesterdayBranch;
      const yesterdayOnToday =
        yesterdayStemUpper === todayStemLodge && yesterdayBranchUpper === dayBranch;
  ...
  },
  {
    ...
    name: "九丑课",
    group: "特殊",
    description: "九丑十日占课，天盘丑加临日支。刚日男凶、柔日女祸。",
    check: r => {
      const dayStem = r.fourPillars.dayStem;
      const dayBranch = r.fourPillars.dayBranch;
      // 九丑十日
  ...
  },
  {
    ...
    name: "伏殃课",
    group: "特殊",
    description: "四课见丧门、吊客（年支前后二位）。主孝服、丧事。",
    check: r => {
      // 丧门：年支+2，吊客：年支-2（对冲前/后）
      const yearBranch = r.fourPillars.yearBranch;
      const sangMen = (yearBranch + 2) % 12;
      const diaoKe = (yearBranch + 10) % 12; // 即 -2
      return inFourLessons(sangMen, r) || inFourLessons(diaoKe, r);
    },
  },
  ...
  // ──── 补充课经规则 ────
  {
    ...
    name: "连珠课",
    group: "三传",
    description: "三传连续相生（初生中、中生末），如连珠不绝。主事情连绵不断、顺遂无阻。",
  ...
  },
  {
    ...
    name: "连茹课",
    group: "三传",
    description: "三传地支连续相同或形成三合局。主事情纠缠、连绵不绝。",
    check: r => {
      const { initial, middle, final } = r.threeTransmissions;
      // 三传相同
  ...
  },
  {
    ...
    name: "天心课",
    group: "特殊",
    description: "四建（太岁、月建、日支、占时）尽入四课整体地支集合。主非常之事可即日而成。",
    check: r => {
  ...
  },
  {
    ...
    name: "天目课",
    group: "特殊",
    description: "天魁戌乘朱雀发用。主文书口舌、眼目之疾。",
    check: r => {
      const initial = r.threeTransmissions.initial;
  ...
  },
  {
    ...
    name: "天耳课",
    group: "特殊",
    description: "天魁戌乘太阴发用。主暗中听闻、机密之事。",
    check: r => {
      const initial = r.threeTransmissions.initial;
      if (initial !== 10) return false; // 戌
      const initialGeneral = getGeneralRidingBranch(initial, r);
  ...
  },
  {
    ...
    name: "金华课",
    group: "特殊",
    description: "太白酉乘太阴或天后发用。主阴私暗昧、女子之事。",
    check: r => {
      const initial = r.threeTransmissions.initial;
  ...
  },
  {
    ...
    name: "玉堂课",
    group: "特殊",
    description: "日干寄宫上神乘贵人或青龙。主文章显达、科甲高中。",
    check: r => {
      const ganShang = r.fourLessons[0]?.upper;
  ...
  },
  {
    ...
    name: "进儒课",
    group: "特殊",
    description: "日干寄宫上神得旺相且乘吉将，发用又生之日干。主学业进步、科举得中。",
    check: r => {
      const dayStem = r.fourPillars.dayStem;
      const ganShang = r.fourLessons[0]?.upper;
      if (ganShang === undefined) return false;
  ...
  },
  {
    ...
    name: "退儒课",
    group: "特殊",
    description: "日干寄宫上神休囚且乘凶将，发用又克之日干。主学业退步、科举落第。",
    check: r => {
      const dayStem = r.fourPillars.dayStem;
      const ganShang = r.fourLessons[0]?.upper;
      if (ganShang === undefined) return false;
  ...
  },
  {
    ...
    name: "龙虎课",
    group: "天将",
    description: "青龙与白虎同入三传，或分临初末传。主竞争激烈的变动。",
    check: r => {
      const { initial, middle, final } = r.threeTransmissions;
      const generals = [initial, middle, final].map(b => getGeneralRidingBranch(b, r));
  ...
  },
  {
    ...
    name: "天罗地网课",
    group: "特殊",
    description: "四课中见天罗（戌）与地网（辰）。主困厄难出、事多阻碍。",
    check: r => {
