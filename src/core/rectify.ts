 ...
 * 流派/四化表/子时界沿用当前输入设置，盘型一律天盘（地/人盘为重排视角，不参与校时）。
 * 确定性推演，仅供缩小范围参考。
 */
import { astro } from "iztro";
import type { GenderName } from "iztro/lib/i18n";
import type { Astrolabe, BirthInput, DecadeInfo } from "./useZwds";
import { MUTAGEN_TABLES, TIME_OPTIONS, bodyPalaceBranchOf } from "./utils";
...
/** 同日十三时辰并排起盘（不作真太阳时校正；流派/四化表沿用输入，盘型一律天盘） */
  ...
  for (const t of TIME_OPTIONS) {
    ...
    try {
      chart = astro.withOptions({
        ...
        fixLeap: true,
        language: "zh-CN",
        // 校时一律以天盘为基准：地/人盘是同一生辰的重排视角，时辰对了天盘才对，
        // 用重排盘的命宫核对性格特征会偏靶
        astroType: "heaven",
        config: {
    ...
    out.push({
      ...
      bodyBranch: bodyPalaceBranchOf(chart.palaces, chart.earthlyBranchOfBodyPalace as string),
