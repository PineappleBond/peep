export interface DistrictInfo {
  name: string;
  longitude: number;
}

export interface CityData {
  name: string;
  districts: DistrictInfo[];
}

export interface ProvinceData {
  name: string;
  cities: CityData[];
}

function c(name: string, districts: [string, number][]): CityData {
  return { name, districts: districts.map(([n, lng]) => ({ name: n, longitude: lng })) };
}

export const PROVINCES: ProvinceData[] = [
  {
    name: "北京",
    cities: [
      c("北京", [
        ["市区", 116.41],
        ["东城区", 116.42],
        ["西城区", 116.37],
        ["朝阳区", 116.45],
        ["丰台区", 116.29],
        ["石景山区", 116.23],
        ["海淀区", 116.31],
        ["门头沟区", 116.11],
        ["房山区", 116.15],
        ["通州区", 116.66],
        ["顺义区", 116.66],
        ["昌平区", 116.24],
        ["大兴区", 116.35],
        ["怀柔区", 116.64],
        ["平谷区", 117.13],
        ["密云区", 116.92],
        ["延庆区", 115.99],
      ]),
    ],
  },
  {
    name: "天津",
    cities: [
      c("天津", [
        ["市区", 117.21],
        ["和平区", 117.22],
        ["河东区", 117.26],
        ["河西区", 117.23],
        ["南开区", 117.16],
        ["河北区", 117.2],
        ["红桥区", 117.16],
        ["东丽区", 117.32],
        ["西青区", 117.01],
        ["津南区", 117.36],
        ["北辰区", 117.14],
        ["武清区", 117.05],
        ["宝坻区", 117.32],
        ["滨海新区", 117.72],
        ["宁河区", 117.83],
        ["静海区", 116.98],
        ["蓟州区", 117.41],
      ]),
    ],
  },
  {
    name: "河北",
    cities: [
      c("石家庄", [
        ["市区", 114.47],
        ["长安区", 114.55],
        ["桥西区", 114.47],
        ["新华区", 114.47],
        ["井陉矿区", 114.07],
        ["裕华区", 114.54],
        ["藁城区", 114.85],
        ["鹿泉区", 114.32],
        ["栾城区", 114.65],
        ["井陉县", 114.15],
        ["正定县", 114.58],
        ["行唐县", 114.56],
        ["灵寿县", 114.39],
        ["高邑县", 114.62],
        ["深泽县", 115.21],
        ["赞皇县", 114.39],
        ["无极县", 114.98],
        ["平山县", 113.99],
        ["元氏县", 114.53],
        ["赵县", 114.78],
        ["石家庄高新技术产业开发区", 114.65],
        ["石家庄循环化工园区", 114.7],
        ["辛集市", 115.22],
        ["晋州市", 115.05],
        ["新乐市", 114.69],
      ]),
      c("唐山", [
        ["市区", 118.19],
        ["路南区", 118.16],
        ["路北区", 118.21],
        ["古冶区", 118.45],
        ["开平区", 118.27],
        ["丰南区", 118.09],
        ["丰润区", 118.17],
        ["曹妃甸区", 118.47],
        ["滦南县", 118.68],
        ["乐亭县", 118.92],
        ["迁西县", 118.32],
        ["玉田县", 117.75],
        ["河北唐山芦台经济开发区", 118.19],
        ["唐山市汉沽管理区", 118.19],
        ["唐山高新技术产业开发区", 118.19],
        ["河北唐山海港经济开发区", 119.62],
        ["遵化市", 117.97],
        ["迁安市", 118.71],
        ["滦州市", 118.71],
      ]),
      c("秦皇岛", [
        ["市区", 119.61],
        ["海港区", 119.62],
        ["山海关区", 119.78],
        ["北戴河区", 119.49],
        ["抚宁区", 119.25],
        ["青龙满族自治县", 118.96],
        ["昌黎县", 119.17],
        ["卢龙县", 118.9],
        ["秦皇岛市经济技术开发区", 119.61],
        ["北戴河新区", 119.42],
      ]),
      c("邯郸", [
        ["市区", 114.55],
        ["邯山区", 114.5],
        ["丛台区", 114.5],
        ["复兴区", 114.48],
        ["峰峰矿区", 114.22],
        ["肥乡区", 114.81],
        ["永年区", 114.55],
        ["临漳县", 114.63],
        ["成安县", 114.68],
        ["大名县", 115.15],
        ["涉县", 113.7],
        ["磁县", 114.38],
        ["邱县", 115.19],
        ["鸡泽县", 114.9],
        ["广平县", 114.95],
        ["馆陶县", 115.29],
        ["魏县", 114.95],
        ["曲周县", 114.96],
        ["邯郸经济技术开发区", 114.57],
        ["邯郸冀南新区", 114.47],
        ["武安市", 114.21],
      ]),
      c("邢台", [
        ["市区", 115.03],
        ["桥东区", 114.51],
        ["桥西区", 114.47],
        ["邢台县", 114.14],
        ["临城县", 114.51],
        ["内丘县", 114.52],
        ["柏乡县", 114.7],
        ["隆尧县", 114.78],
        ["任县", 114.68],
        ["南和县", 114.69],
        ["宁晋县", 114.93],
        ["巨鹿县", 115.04],
        ["新河县", 115.25],
        ["广宗县", 115.15],
        ["平乡县", 115.04],
        ["威县", 115.27],
        ["清河县", 115.67],
        ["临西县", 115.51],
        ["河北邢台经济开发区", 114.57],
        ["南宫市", 115.41],
        ["沙河市", 114.51],
      ]),
      c("保定", [
        ["市区", 115.47],
        ["竞秀区", 115.47],
        ["莲池区", 115.54],
        ["满城区", 115.33],
        ["清苑区", 115.5],
        ["徐水区", 115.66],
        ["涞水县", 115.72],
        ["阜平县", 114.2],
        ["定兴县", 115.81],
        ["唐县", 114.99],
        ["高阳县", 115.79],
        ["容城县", 115.87],
        ["涞源县", 114.7],
        ["望都县", 115.16],
        ["安新县", 115.94],
        ["易县", 115.5],
        ["曲阳县", 114.75],
        ["蠡县", 115.59],
        ["顺平县", 115.14],
        ["博野县", 115.47],
        ["雄县", 116.11],
        ["保定高新技术产业开发区", 116.05],
        ["保定白沟新城", 116.05],
        ["涿州市", 115.98],
        ["定州市", 115.0],
        ["安国市", 115.33],
        ["高碑店市", 115.88],
      ]),
      c("张家口", [
        ["市区", 114.89],
        ["桥东区", 114.9],
        ["桥西区", 114.88],
        ["宣化区", 115.11],
        ["下花园区", 115.29],
        ["万全区", 114.75],
        ["崇礼区", 115.29],
        ["张北县", 114.73],
        ["康保县", 114.61],
        ["沽源县", 115.7],
        ["尚义县", 113.98],
        ["蔚县", 114.6],
        ["阳原县", 114.16],
        ["怀安县", 114.39],
        ["怀来县", 115.52],
        ["涿鹿县", 115.21],
        ["赤城县", 115.84],
        ["张家口经济开发区", 114.89],
        ["张家口市察北管理区", 114.95],
        ["张家口市塞北管理区", 115.83],
      ]),
      c("承德", [
        ["市区", 117.97],
        ["双桥区", 117.95],
        ["双滦区", 117.81],
        ["鹰手营子矿区", 117.67],
        ["承德县", 118.18],
        ["兴隆县", 117.51],
        ["滦平县", 117.34],
        ["隆化县", 117.75],
        ["丰宁满族自治县", 116.65],
        ["宽城满族自治县", 118.49],
        ["围场满族蒙古族自治县", 117.77],
        ["承德高新技术产业开发区", 118.29],
        ["平泉市", 118.71],
      ]),
      c("沧州", [
        ["市区", 116.85],
        ["新华区", 116.87],
        ["运河区", 116.84],
        ["沧县", 116.88],
        ["青县", 116.81],
        ["东光县", 116.54],
        ["海兴县", 117.5],
        ["盐山县", 117.24],
        ["肃宁县", 115.84],
        ["南皮县", 116.72],
        ["吴桥县", 116.4],
        ["献县", 116.13],
        ["孟村回族自治县", 117.11],
        ["河北沧州经济开发区", 117.2],
        ["沧州高新技术产业开发区", 116.82],
        ["沧州渤海新区", 117.76],
        ["泊头市", 116.58],
        ["任丘市", 116.11],
        ["黄骅市", 117.34],
        ["河间市", 116.11],
      ]),
      c("廊坊", [
        ["市区", 116.69],
        ["安次区", 116.71],
        ["广阳区", 116.72],
        ["固安县", 116.31],
        ["永清县", 116.51],
        ["香河县", 117.01],
        ["大城县", 116.66],
        ["文安县", 116.46],
        ["大厂回族自治县", 117.0],
        ["廊坊经济技术开发区", 116.69],
        ["霸州市", 116.4],
        ["三河市", 117.08],
      ]),
      c("衡水", [
        ["市区", 115.68],
        ["桃城区", 115.68],
        ["冀州区", 115.59],
        ["枣强县", 115.73],
        ["武邑县", 115.89],
        ["武强县", 115.99],
        ["饶阳县", 115.73],
        ["安平县", 115.53],
        ["故城县", 115.97],
        ["景县", 116.28],
        ["阜城县", 116.18],
        ["河北衡水高新技术产业开发区", 117.2],
        ["衡水滨湖新区", 115.71],
        ["深州市", 115.57],
      ]),
    ],
  },
  {
    name: "山西",
    cities: [
      c("太原", [
        ["市区", 112.56],
        ["小店区", 112.57],
        ["迎泽区", 112.57],
        ["杏花岭区", 112.58],
        ["尖草坪区", 112.49],
        ["万柏林区", 112.52],
        ["晋源区", 112.48],
        ["清徐县", 112.36],
        ["阳曲县", 112.68],
        ["娄烦县", 111.8],
        ["山西转型综合改革示范区", 112.6],
        ["古交市", 112.18],
      ]),
      c("大同", [
        ["市区", 113.31],
        ["新荣区", 113.15],
        ["平城区", 113.3],
        ["云冈区", 113.16],
        ["云州区", 113.62],
        ["阳高县", 113.76],
        ["天镇县", 114.1],
        ["广灵县", 114.29],
        ["灵丘县", 114.24],
        ["浑源县", 113.71],
        ["左云县", 112.71],
        ["山西大同经济开发区", 113.4],
      ]),
      c("阳泉", [
        ["市区", 113.59],
        ["城区", 113.61],
        ["矿区", 113.56],
        ["郊区", 113.6],
        ["平定县", 113.66],
        ["盂县", 113.42],
      ]),
      c("长治", [
        ["市区", 113.12],
        ["潞州区", 113.13],
        ["上党区", 113.06],
        ["屯留区", 112.9],
        ["潞城区", 113.24],
        ["襄垣县", 113.06],
        ["平顺县", 113.44],
        ["黎城县", 113.39],
        ["壶关县", 113.21],
        ["长子县", 112.88],
        ["武乡县", 112.87],
        ["沁县", 112.71],
        ["沁源县", 112.34],
        ["山西长治高新技术产业园区", 113.12],
      ]),
      c("晋城", [
        ["市区", 112.86],
        ["城区", 112.86],
        ["沁水县", 112.19],
        ["阳城县", 112.42],
        ["陵川县", 113.29],
        ["泽州县", 112.79],
        ["高平市", 112.93],
      ]),
      c("朔州", [
        ["市区", 112.44],
        ["朔城区", 112.44],
        ["平鲁区", 112.3],
        ["山阴县", 112.82],
        ["应县", 113.2],
        ["右玉县", 112.47],
        ["山西朔州经济开发区", 112.44],
        ["怀仁市", 113.11],
      ]),
      c("晋中", [
        ["市区", 112.76],
        ["榆次区", 112.71],
        ["榆社县", 112.98],
        ["左权县", 113.39],
        ["和顺县", 113.58],
        ["昔阳县", 113.71],
        ["寿阳县", 113.18],
        ["太谷县", 112.56],
        ["祁县", 112.34],
        ["平遥县", 112.18],
        ["灵石县", 111.79],
        ["介休市", 111.92],
      ]),
      c("运城", [
        ["市区", 111.01],
        ["盐湖区", 111.0],
        ["临猗县", 110.78],
        ["万荣县", 110.84],
        ["闻喜县", 111.23],
        ["稷山县", 110.99],
        ["新绛县", 111.23],
        ["绛县", 111.58],
        ["垣曲县", 111.68],
        ["夏县", 111.23],
        ["平陆县", 111.22],
        ["芮城县", 110.7],
        ["永济市", 110.45],
        ["河津市", 110.72],
      ]),
      c("忻州", [
        ["市区", 112.74],
        ["忻府区", 112.75],
        ["定襄县", 112.96],
        ["五台县", 113.26],
        ["代县", 112.97],
        ["繁峙县", 113.27],
        ["宁武县", 112.33],
        ["静乐县", 111.95],
        ["神池县", 112.22],
        ["五寨县", 111.85],
        ["岢岚县", 111.58],
        ["河曲县", 111.14],
        ["保德县", 111.09],
        ["偏关县", 111.51],
        ["五台山风景名胜区", 113.6],
        ["原平市", 112.72],
      ]),
      c("临汾", [
        ["市区", 111.53],
        ["尧都区", 111.59],
        ["曲沃县", 111.48],
        ["翼城县", 111.73],
        ["襄汾县", 111.45],
        ["洪洞县", 111.68],
        ["古县", 111.93],
        ["安泽县", 112.26],
        ["浮山县", 111.86],
        ["吉县", 110.69],
        ["乡宁县", 110.85],
        ["大宁县", 110.76],
        ["隰县", 110.95],
        ["永和县", 110.64],
        ["蒲县", 111.1],
        ["汾西县", 111.57],
        ["侯马市", 111.38],
        ["霍州市", 111.76],
      ]),
      c("吕梁", [
        ["市区", 111.15],
        ["离石区", 111.16],
        ["文水县", 112.04],
        ["交城县", 112.16],
        ["兴县", 111.13],
        ["临县", 111.0],
        ["柳林县", 110.9],
        ["石楼县", 110.84],
        ["岚县", 111.68],
        ["方山县", 111.25],
        ["中阳县", 111.19],
        ["交口县", 111.19],
        ["孝义市", 111.79],
        ["汾阳市", 111.79],
      ]),
    ],
  },
  {
    name: "内蒙古",
    cities: [
      c("呼和浩特", [
        ["市区", 111.76],
        ["新城区", 111.67],
        ["回民区", 111.63],
        ["玉泉区", 111.68],
        ["赛罕区", 111.71],
        ["土默特左旗", 111.17],
        ["托克托县", 111.2],
        ["和林格尔县", 111.83],
        ["清水河县", 111.65],
        ["武川县", 111.46],
        ["呼和浩特金海工业园区", 111.61],
        ["呼和浩特经济技术开发区", 111.55],
      ]),
      c("包头", [
        ["市区", 109.85],
        ["东河区", 110.05],
        ["昆都仑区", 109.85],
        ["青山区", 109.91],
        ["石拐区", 110.28],
        ["白云鄂博矿区", 109.98],
        ["九原区", 109.97],
        ["土默特右旗", 110.53],
        ["固阳县", 110.07],
        ["达尔罕茂明安联合旗", 110.44],
        ["包头稀土高新技术产业开发区", 109.85],
      ]),
      c("乌海", [
        ["市区", 106.8],
        ["海勃湾区", 106.83],
        ["海南区", 106.9],
        ["乌达区", 106.73],
      ]),
      c("赤峰", [
        ["市区", 118.9],
        ["红山区", 118.96],
        ["元宝山区", 119.3],
        ["松山区", 118.94],
        ["阿鲁科尔沁旗", 120.07],
        ["巴林左旗", 119.39],
        ["巴林右旗", 118.67],
        ["林西县", 118.06],
        ["克什克腾旗", 117.55],
        ["翁牛特旗", 119.01],
        ["喀喇沁旗", 118.71],
        ["宁城县", 119.35],
        ["敖汉旗", 119.93],
      ]),
      c("通辽", [
        ["市区", 122.25],
        ["科尔沁区", 122.26],
        ["科尔沁左翼中旗", 122.26],
        ["科尔沁左翼后旗", 122.26],
        ["开鲁县", 121.33],
        ["库伦旗", 121.82],
        ["奈曼旗", 120.66],
        ["扎鲁特旗", 120.92],
        ["通辽经济技术开发区", 122.26],
        ["霍林郭勒市", 119.67],
      ]),
      c("鄂尔多斯", [
        ["市区", 109.79],
        ["东胜区", 109.97],
        ["康巴什区", 109.87],
        ["达拉特旗", 110.04],
        ["准格尔旗", 111.25],
        ["鄂托克前旗", 107.48],
        ["鄂托克旗", 107.98],
        ["杭锦旗", 108.74],
        ["乌审旗", 108.82],
        ["伊金霍洛旗", 109.75],
      ]),
      c("呼伦贝尔", [
        ["市区", 119.77],
        ["海拉尔区", 119.74],
        ["扎赉诺尔区", 117.69],
        ["阿荣旗", 123.47],
        ["莫力达瓦达斡尔族自治旗", 124.53],
        ["鄂伦春自治旗", 123.73],
        ["鄂温克族自治旗", 119.76],
        ["陈巴尔虎旗", 119.43],
        ["新巴尔虎左旗", 118.28],
        ["新巴尔虎右旗", 116.83],
        ["满洲里市", 117.39],
        ["牙克石市", 120.72],
        ["扎兰屯市", 122.74],
        ["额尔古纳市", 120.19],
        ["根河市", 121.53],
      ]),
      c("巴彦淖尔", [
        ["市区", 107.39],
        ["临河区", 107.37],
        ["五原县", 108.27],
        ["磴口县", 107.01],
        ["乌拉特前旗", 108.66],
        ["乌拉特中旗", 108.52],
        ["乌拉特后旗", 107.08],
        ["杭锦后旗", 107.16],
      ]),
      c("乌兰察布", [
        ["市区", 113.14],
        ["集宁区", 113.12],
        ["卓资县", 112.58],
        ["化德县", 114.02],
        ["商都县", 113.58],
        ["兴和县", 113.84],
        ["凉城县", 112.51],
        ["察哈尔右翼前旗", 113.22],
        ["察哈尔右翼中旗", 112.64],
        ["察哈尔右翼后旗", 113.2],
        ["四子王旗", 111.71],
        ["丰镇市", 113.12],
      ]),
      c("兴安盟", [
        ["市区", 122.04],
        ["乌兰浩特市", 122.1],
        ["阿尔山市", 119.95],
        ["科尔沁右翼前旗", 121.96],
        ["科尔沁右翼中旗", 121.48],
        ["扎赉特旗", 122.91],
        ["突泉县", 121.6],
      ]),
      c("锡林郭勒盟", [
        ["市区", 116.05],
        ["二连浩特市", 111.96],
        ["锡林浩特市", 116.09],
        ["阿巴嘎旗", 114.96],
        ["苏尼特左旗", 113.67],
        ["苏尼特右旗", 112.65],
        ["东乌珠穆沁旗", 116.98],
        ["西乌珠穆沁旗", 117.62],
        ["太仆寺旗", 115.29],
        ["镶黄旗", 113.85],
        ["正镶白旗", 115.04],
        ["正蓝旗", 116.0],
        ["多伦县", 116.49],
        ["乌拉盖管委会", 114.1],
      ]),
      c("阿拉善盟", [
        ["市区", 105.74],
        ["阿拉善左旗", 105.67],
        ["阿拉善右旗", 101.67],
        ["额济纳旗", 101.06],
        ["内蒙古阿拉善经济开发区", 105.74],
      ]),
    ],
  },
  {
    name: "辽宁",
    cities: [
      c("沈阳", [
        ["市区", 123.47],
        ["和平区", 123.43],
        ["沈河区", 123.47],
        ["大东区", 123.48],
        ["皇姑区", 123.43],
        ["铁西区", 123.38],
        ["苏家屯区", 123.35],
        ["浑南区", 123.46],
        ["沈北新区", 123.53],
        ["于洪区", 123.31],
        ["辽中区", 122.77],
        ["康平县", 123.34],
        ["法库县", 123.41],
        ["新民市", 122.88],
      ]),
      c("大连", [
        ["市区", 121.62],
        ["中山区", 121.65],
        ["西岗区", 121.62],
        ["沙河口区", 121.59],
        ["甘井子区", 121.53],
        ["旅顺口区", 121.27],
        ["金州区", 121.79],
        ["普兰店区", 121.95],
        ["长海县", 122.59],
        ["瓦房店市", 121.99],
        ["庄河市", 122.97],
      ]),
      c("鞍山", [
        ["市区", 123.0],
        ["铁东区", 123.0],
        ["铁西区", 122.98],
        ["立山区", 123.04],
        ["千山区", 122.97],
        ["台安县", 122.44],
        ["岫岩满族自治县", 123.29],
        ["海城市", 122.69],
      ]),
      c("抚顺", [
        ["市区", 123.96],
        ["新抚区", 123.92],
        ["东洲区", 124.05],
        ["望花区", 123.79],
        ["顺城区", 123.95],
        ["抚顺县", 123.92],
        ["新宾满族自治县", 125.05],
        ["清原满族自治县", 124.93],
      ]),
      c("本溪", [
        ["市区", 123.69],
        ["平山区", 123.77],
        ["溪湖区", 123.77],
        ["明山区", 123.82],
        ["南芬区", 123.75],
        ["本溪满族自治县", 124.13],
        ["桓仁满族自治县", 125.37],
      ]),
      c("丹东", [
        ["市区", 124.36],
        ["元宝区", 124.4],
        ["振兴区", 124.33],
        ["振安区", 124.43],
        ["宽甸满族自治县", 124.79],
        ["东港市", 124.16],
        ["凤城市", 124.07],
      ]),
      c("锦州", [
        ["市区", 121.13],
        ["古塔区", 121.13],
        ["凌河区", 121.16],
        ["太和区", 121.11],
        ["黑山县", 122.13],
        ["义县", 121.25],
        ["凌海市", 121.36],
        ["北镇市", 121.78],
      ]),
      c("营口", [
        ["市区", 122.24],
        ["站前区", 122.27],
        ["西市区", 122.21],
        ["鲅鱼圈区", 122.13],
        ["老边区", 122.39],
        ["盖州市", 122.36],
        ["大石桥市", 122.52],
      ]),
      c("阜新", [
        ["市区", 121.68],
        ["海州区", 121.67],
        ["新邱区", 121.8],
        ["太平区", 121.69],
        ["清河门区", 121.42],
        ["细河区", 121.69],
        ["阜新蒙古族自治县", 121.76],
        ["彰武县", 122.55],
      ]),
      c("辽阳", [
        ["市区", 123.24],
        ["白塔区", 123.18],
        ["文圣区", 123.24],
        ["宏伟区", 123.2],
        ["弓长岭区", 123.43],
        ["太子河区", 123.19],
        ["辽阳县", 123.11],
        ["灯塔市", 123.35],
      ]),
      c("盘锦", [
        ["市区", 122.08],
        ["双台子区", 122.05],
        ["兴隆台区", 122.08],
        ["大洼区", 122.09],
        ["盘山县", 122.0],
      ]),
      c("铁岭", [
        ["市区", 123.73],
        ["银州区", 123.85],
        ["清河区", 124.17],
        ["铁岭县", 123.74],
        ["西丰县", 124.73],
        ["昌图县", 124.12],
        ["调兵山市", 123.57],
        ["开原市", 124.04],
      ]),
      c("朝阳", [
        ["市区", 120.46],
        ["双塔区", 120.46],
        ["龙城区", 120.44],
        ["朝阳县", 120.26],
        ["建平县", 119.65],
        ["喀喇沁左翼蒙古族自治县", 119.75],
        ["北票市", 120.78],
        ["凌源市", 119.41],
      ]),
      c("葫芦岛", [
        ["市区", 120.84],
        ["连山区", 120.88],
        ["龙港区", 120.9],
        ["南票区", 120.76],
        ["绥中县", 120.35],
        ["建昌县", 119.84],
        ["兴城市", 120.76],
      ]),
    ],
  },
  {
    name: "吉林",
    cities: [
      c("长春", [
        ["市区", 125.33],
        ["南关区", 125.36],
        ["宽城区", 125.33],
        ["朝阳区", 125.3],
        ["二道区", 125.38],
        ["绿园区", 125.26],
        ["双阳区", 125.67],
        ["九台区", 125.85],
        ["农安县", 125.19],
        ["长春经济技术开发区", 125.43],
        ["长春净月高新技术产业开发区", 125.4],
        ["长春高新技术产业开发区", 125.26],
        ["长春汽车经济技术开发区", 125.26],
        ["榆树市", 126.54],
        ["德惠市", 125.73],
      ]),
      c("吉林市", [
        ["市区", 126.56],
        ["昌邑区", 126.58],
        ["龙潭区", 126.57],
        ["船营区", 126.55],
        ["丰满区", 126.57],
        ["永吉县", 126.5],
        ["吉林经济开发区", 126.45],
        ["吉林高新技术产业开发区", 126.61],
        ["吉林中国新加坡食品区", 125.99],
        ["蛟河市", 127.35],
        ["桦甸市", 126.75],
        ["舒兰市", 126.97],
        ["磐石市", 126.07],
      ]),
      c("四平", [
        ["市区", 124.36],
        ["铁西区", 124.35],
        ["铁东区", 124.42],
        ["梨树县", 124.34],
        ["伊通满族自治县", 125.31],
        ["公主岭市", 124.83],
        ["双辽市", 123.51],
      ]),
      c("辽源", [
        ["市区", 125.15],
        ["龙山区", 125.14],
        ["西安区", 125.16],
        ["东丰县", 125.54],
        ["东辽县", 125.0],
      ]),
      c("通化", [
        ["市区", 125.95],
        ["东昌区", 125.93],
        ["二道江区", 126.05],
        ["通化县", 125.77],
        ["辉南县", 126.05],
        ["柳河县", 125.75],
        ["梅河口市", 125.72],
        ["集安市", 126.2],
      ]),
      c("白山", [
        ["市区", 126.43],
        ["浑江区", 126.42],
        ["江源区", 126.6],
        ["抚松县", 127.46],
        ["靖宇县", 126.82],
        ["长白朝鲜族自治县", 128.21],
        ["临江市", 126.92],
      ]),
      c("松原", [
        ["市区", 124.83],
        ["宁江区", 124.82],
        ["前郭尔罗斯蒙古族自治县", 124.83],
        ["长岭县", 123.97],
        ["乾安县", 124.05],
        ["吉林松原经济开发区", 124.83],
        ["扶余市", 126.06],
      ]),
      c("白城", [
        ["市区", 122.85],
        ["洮北区", 122.86],
        ["镇赉县", 123.21],
        ["通榆县", 123.09],
        ["吉林白城经济开发区", 122.85],
        ["洮南市", 122.79],
        ["大安市", 124.3],
      ]),
      c("延边朝鲜族自治州", [
        ["市区", 129.48],
        ["延吉市", 129.52],
        ["图们市", 129.85],
        ["敦化市", 128.24],
        ["珲春市", 130.37],
        ["龙井市", 129.43],
        ["和龙市", 129.02],
        ["汪清县", 129.78],
        ["安图县", 128.91],
      ]),
    ],
  },
  {
    name: "黑龙江",
    cities: [
      c("哈尔滨", [
        ["市区", 126.54],
        ["道里区", 126.62],
        ["南岗区", 126.68],
        ["道外区", 126.66],
        ["平房区", 126.64],
        ["松北区", 126.52],
        ["香坊区", 126.67],
        ["呼兰区", 126.59],
        ["阿城区", 126.96],
        ["双城区", 126.32],
        ["依兰县", 129.57],
        ["方正县", 128.84],
        ["宾县", 127.47],
        ["巴彦县", 127.41],
        ["木兰县", 128.05],
        ["通河县", 128.75],
        ["延寿县", 128.34],
        ["尚志市", 128.02],
        ["五常市", 127.17],
      ]),
      c("齐齐哈尔", [
        ["市区", 123.92],
        ["龙沙区", 123.96],
        ["建华区", 123.96],
        ["铁锋区", 123.98],
        ["昂昂溪区", 123.83],
        ["富拉尔基区", 123.64],
        ["碾子山区", 122.89],
        ["梅里斯达斡尔族区", 123.76],
        ["龙江县", 123.21],
        ["依安县", 125.31],
        ["泰来县", 123.42],
        ["甘南县", 123.51],
        ["富裕县", 124.48],
        ["克山县", 125.88],
        ["克东县", 126.26],
        ["拜泉县", 126.11],
        ["讷河市", 124.89],
      ]),
      c("鸡西", [
        ["市区", 130.98],
        ["鸡冠区", 130.99],
        ["恒山区", 130.91],
        ["滴道区", 130.85],
        ["梨树区", 130.7],
        ["城子河区", 131.02],
        ["麻山区", 130.48],
        ["鸡东县", 131.13],
        ["虎林市", 132.94],
        ["密山市", 131.85],
      ]),
      c("鹤岗", [
        ["市区", 130.3],
        ["向阳区", 130.3],
        ["工农区", 130.28],
        ["南山区", 130.29],
        ["兴安区", 130.25],
        ["东山区", 130.32],
        ["兴山区", 130.32],
        ["萝北县", 130.84],
        ["绥滨县", 131.86],
      ]),
      c("双鸭山", [
        ["市区", 131.17],
        ["尖山区", 131.17],
        ["岭东区", 131.16],
        ["四方台区", 131.35],
        ["宝山区", 131.41],
        ["集贤县", 131.15],
        ["友谊县", 131.81],
        ["宝清县", 132.2],
        ["饶河县", 134.02],
      ]),
      c("大庆", [
        ["市区", 125.11],
        ["萨尔图区", 125.13],
        ["龙凤区", 125.14],
        ["让胡路区", 124.88],
        ["红岗区", 124.9],
        ["大同区", 124.82],
        ["肇州县", 125.28],
        ["肇源县", 125.08],
        ["林甸县", 124.87],
        ["杜尔伯特蒙古族自治县", 124.45],
        ["大庆高新技术产业开发区", 125.16],
      ]),
      c("伊春", [
        ["市区", 128.85],
        ["伊美区", 128.85],
        ["乌翠区", 128.85],
        ["友好区", 128.84],
        ["嘉荫县", 130.41],
        ["汤旺县", 129.04],
        ["丰林县", 129.03],
        ["大箐山县", 129.01],
        ["南岔县", 129.29],
        ["金林区", 128.85],
        ["铁力市", 128.04],
      ]),
      c("佳木斯", [
        ["市区", 130.33],
        ["向阳区", 130.37],
        ["前进区", 130.38],
        ["东风区", 130.41],
        ["郊区", 130.33],
        ["桦南县", 130.56],
        ["桦川县", 130.73],
        ["汤原县", 129.91],
        ["同江市", 132.52],
        ["富锦市", 132.04],
        ["抚远市", 134.31],
      ]),
      c("七台河", [
        ["市区", 131.01],
        ["新兴区", 130.94],
        ["桃山区", 131.03],
        ["茄子河区", 131.07],
        ["勃利县", 130.6],
      ]),
      c("牡丹江", [
        ["市区", 129.64],
        ["东安区", 129.63],
        ["阳明区", 129.64],
        ["爱民区", 129.6],
        ["西安区", 129.62],
        ["林口县", 130.29],
        ["牡丹江经济技术开发区", 129.39],
        ["绥芬河市", 131.16],
        ["海林市", 129.39],
        ["宁安市", 129.49],
        ["穆棱市", 130.53],
        ["东宁市", 131.13],
      ]),
      c("黑河", [
        ["市区", 127.54],
        ["爱辉区", 127.51],
        ["逊克县", 128.49],
        ["孙吴县", 127.34],
        ["北安市", 126.5],
        ["五大连池市", 126.21],
        ["嫩江市", 125.23],
      ]),
      c("绥化", [
        ["市区", 126.98],
        ["北林区", 126.99],
        ["望奎县", 126.49],
        ["兰西县", 126.29],
        ["青冈县", 126.1],
        ["庆安县", 127.51],
        ["明水县", 125.91],
        ["绥棱县", 127.12],
        ["安达市", 125.35],
        ["肇东市", 125.97],
        ["海伦市", 126.94],
      ]),
      c("大兴安岭地区", [
        ["市区", 124.15],
        ["漠河市", 122.55],
        ["呼玛县", 126.66],
        ["塔河县", 124.72],
        ["加格达奇区", 124.15],
        ["松岭区", 124.15],
        ["新林区", 124.54],
        ["呼中区", 123.58],
      ]),
    ],
  },
  {
    name: "上海",
    cities: [
      c("上海", [
        ["市区", 121.48],
        ["黄浦区", 121.49],
        ["徐汇区", 121.44],
        ["长宁区", 121.43],
        ["静安区", 121.45],
        ["普陀区", 121.4],
        ["虹口区", 121.51],
        ["杨浦区", 121.53],
        ["闵行区", 121.39],
        ["宝山区", 121.5],
        ["嘉定区", 121.27],
        ["浦东新区", 121.55],
        ["金山区", 121.35],
        ["松江区", 121.23],
        ["青浦区", 121.13],
        ["奉贤区", 121.48],
        ["崇明区", 121.41],
      ]),
    ],
  },
  {
    name: "江苏",
    cities: [
      c("南京", [
        ["市区", 118.8],
        ["玄武区", 118.8],
        ["秦淮区", 118.8],
        ["建邺区", 118.74],
        ["鼓楼区", 118.78],
        ["浦口区", 118.63],
        ["栖霞区", 118.92],
        ["雨花台区", 118.79],
        ["江宁区", 118.85],
        ["六合区", 118.83],
        ["溧水区", 119.03],
        ["高淳区", 118.9],
      ]),
      c("无锡", [
        ["市区", 120.32],
        ["锡山区", 120.36],
        ["惠山区", 120.3],
        ["滨湖区", 120.29],
        ["梁溪区", 120.31],
        ["新吴区", 120.37],
        ["江阴市", 120.29],
        ["宜兴市", 119.83],
      ]),
      c("徐州", [
        ["市区", 117.29],
        ["鼓楼区", 117.19],
        ["云龙区", 117.26],
        ["贾汪区", 117.47],
        ["泉山区", 117.2],
        ["铜山区", 117.18],
        ["丰县", 116.6],
        ["沛县", 116.94],
        ["睢宁县", 117.95],
        ["徐州经济技术开发区", 117.33],
        ["新沂市", 118.36],
        ["邳州市", 118.02],
      ]),
      c("常州", [
        ["市区", 119.98],
        ["天宁区", 119.98],
        ["钟楼区", 119.91],
        ["新北区", 119.98],
        ["武进区", 119.95],
        ["金坛区", 119.6],
        ["溧阳市", 119.49],
      ]),
      c("苏州", [
        ["市区", 120.59],
        ["虎丘区", 120.58],
        ["吴中区", 120.64],
        ["相城区", 120.65],
        ["姑苏区", 120.62],
        ["吴江区", 120.65],
        ["苏州工业园区", 120.73],
        ["常熟市", 120.76],
        ["张家港市", 120.56],
        ["昆山市", 120.99],
        ["太仓市", 121.14],
      ]),
      c("南通", [
        ["市区", 120.9],
        ["崇川区", 120.86],
        ["港闸区", 120.82],
        ["通州区", 121.08],
        ["如东县", 121.19],
        ["南通经济技术开发区", 120.96],
        ["启东市", 121.66],
        ["如皋市", 120.58],
        ["海门市", 121.19],
        ["海安市", 120.47],
      ]),
      c("连云港", [
        ["市区", 119.23],
        ["连云区", 119.35],
        ["海州区", 119.17],
        ["赣榆区", 119.18],
        ["东海县", 118.76],
        ["灌云县", 119.25],
        ["灌南县", 119.32],
        ["连云港经济技术开发区", 119.33],
        ["连云港高新技术产业开发区", 118.71],
      ]),
      c("淮安", [
        ["市区", 119.02],
        ["淮安区", 119.15],
        ["淮阴区", 119.04],
        ["清江浦区", 119.03],
        ["洪泽区", 118.88],
        ["涟水县", 119.27],
        ["盱眙县", 118.55],
        ["金湖县", 119.03],
        ["淮安经济技术开发区", 119.15],
      ]),
      c("盐城", [
        ["市区", 120.17],
        ["亭湖区", 120.2],
        ["盐都区", 120.16],
        ["大丰区", 120.51],
        ["响水县", 119.58],
        ["滨海县", 119.83],
        ["阜宁县", 119.81],
        ["射阳县", 120.26],
        ["建湖县", 119.81],
        ["盐城经济技术开发区", 120.22],
        ["东台市", 120.33],
      ]),
      c("扬州", [
        ["市区", 119.42],
        ["广陵区", 119.44],
        ["邗江区", 119.4],
        ["江都区", 119.58],
        ["宝应县", 119.37],
        ["扬州经济技术开发区", 119.41],
        ["仪征市", 119.19],
        ["高邮市", 119.47],
      ]),
      c("镇江", [
        ["市区", 119.43],
        ["京口区", 119.48],
        ["润州区", 119.42],
        ["丹徒区", 119.44],
        ["镇江新区", 119.52],
        ["丹阳市", 119.61],
        ["扬中市", 119.8],
        ["句容市", 119.18],
      ]),
      c("泰州", [
        ["市区", 119.93],
        ["海陵区", 119.93],
        ["高港区", 119.89],
        ["姜堰区", 120.13],
        ["泰州医药高新技术产业开发区", 119.91],
        ["兴化市", 119.86],
        ["靖江市", 120.28],
        ["泰兴市", 120.06],
      ]),
      c("宿迁", [
        ["市区", 118.28],
        ["宿城区", 118.25],
        ["宿豫区", 118.34],
        ["沭阳县", 118.81],
        ["泗阳县", 118.71],
        ["泗洪县", 118.22],
        ["宿迁经济技术开发区", 118.28],
      ]),
    ],
  },
  {
    name: "浙江",
    cities: [
      c("杭州", [
        ["市区", 120.22],
        ["上城区", 120.18],
        ["下城区", 120.19],
        ["江干区", 120.21],
        ["拱墅区", 120.15],
        ["西湖区", 120.14],
        ["滨江区", 120.22],
        ["萧山区", 120.27],
        ["余杭区", 120.31],
        ["富阳区", 119.97],
        ["临安区", 119.73],
        ["桐庐县", 119.7],
        ["淳安县", 119.05],
        ["建德市", 119.29],
      ]),
      c("宁波", [
        ["市区", 121.63],
        ["海曙区", 121.56],
        ["江北区", 121.56],
        ["北仑区", 121.85],
        ["镇海区", 121.72],
        ["鄞州区", 121.56],
        ["奉化区", 121.41],
        ["象山县", 121.88],
        ["宁海县", 121.44],
        ["余姚市", 121.16],
        ["慈溪市", 121.27],
      ]),
      c("温州", [
        ["市区", 120.71],
        ["鹿城区", 120.66],
        ["龙湾区", 120.82],
        ["瓯海区", 120.62],
        ["洞头区", 121.16],
        ["永嘉县", 120.69],
        ["平阳县", 120.57],
        ["苍南县", 120.43],
        ["文成县", 120.1],
        ["泰顺县", 119.72],
        ["温州经济技术开发区", 120.82],
        ["瑞安市", 120.66],
        ["乐清市", 120.99],
        ["龙港市", 120.54],
      ]),
      c("嘉兴", [
        ["市区", 120.76],
        ["南湖区", 120.79],
        ["秀洲区", 120.72],
        ["嘉善县", 120.93],
        ["海盐县", 120.95],
        ["海宁市", 120.69],
        ["平湖市", 121.02],
        ["桐乡市", 120.57],
      ]),
      c("湖州", [
        ["市区", 120.09],
        ["吴兴区", 120.19],
        ["南浔区", 120.42],
        ["德清县", 119.98],
        ["长兴县", 119.92],
        ["安吉县", 119.69],
      ]),
      c("绍兴", [
        ["市区", 120.59],
        ["越城区", 120.59],
        ["柯桥区", 120.45],
        ["上虞区", 120.87],
        ["新昌县", 120.91],
        ["诸暨市", 120.25],
        ["嵊州市", 120.84],
      ]),
      c("金华", [
        ["市区", 119.65],
        ["婺城区", 119.58],
        ["金东区", 119.7],
        ["武义县", 119.82],
        ["浦江县", 119.9],
        ["磐安县", 120.46],
        ["兰溪市", 119.47],
        ["义乌市", 120.08],
        ["东阳市", 120.25],
        ["永康市", 120.05],
      ]),
      c("衢州", [
        ["市区", 118.87],
        ["柯城区", 118.88],
        ["衢江区", 118.97],
        ["常山县", 118.52],
        ["开化县", 118.42],
        ["龙游县", 119.18],
        ["江山市", 118.63],
      ]),
      c("舟山", [
        ["市区", 122.21],
        ["定海区", 122.11],
        ["普陀区", 122.33],
        ["岱山县", 122.23],
        ["嵊泗县", 122.46],
      ]),
      c("台州", [
        ["市区", 121.43],
        ["椒江区", 121.45],
        ["黄岩区", 121.27],
        ["路桥区", 121.37],
        ["三门县", 121.4],
        ["天台县", 121.01],
        ["仙居县", 120.74],
        ["温岭市", 121.39],
        ["临海市", 121.15],
        ["玉环市", 121.24],
      ]),
      c("丽水", [
        ["市区", 119.93],
        ["莲都区", 119.92],
        ["青田县", 120.3],
        ["缙云县", 120.1],
        ["遂昌县", 119.28],
        ["松阳县", 119.49],
        ["云和县", 119.58],
        ["庆元县", 119.07],
        ["景宁畲族自治县", 119.64],
        ["龙泉市", 119.15],
      ]),
    ],
  },
  {
    name: "安徽",
    cities: [
      c("合肥", [
        ["市区", 117.23],
        ["瑶海区", 117.32],
        ["庐阳区", 117.27],
        ["蜀山区", 117.27],
        ["包河区", 117.32],
        ["长丰县", 117.17],
        ["肥东县", 117.48],
        ["肥西县", 117.16],
        ["庐江县", 117.29],
        ["合肥高新技术产业开发区", 117.18],
        ["合肥经济技术开发区", 117.22],
        ["合肥新站高新技术产业开发区", 117.18],
        ["巢湖市", 117.9],
      ]),
      c("芜湖", [
        ["市区", 118.44],
        ["镜湖区", 118.39],
        ["弋江区", 118.38],
        ["鸠江区", 118.4],
        ["三山区", 118.27],
        ["芜湖县", 118.58],
        ["繁昌县", 118.21],
        ["南陵县", 118.34],
        ["无为县", 117.91],
        ["芜湖经济技术开发区", 118.45],
        ["安徽芜湖长江大桥经济开发区", 118.38],
      ]),
      c("蚌埠", [
        ["市区", 117.4],
        ["龙子湖区", 117.4],
        ["蚌山区", 117.38],
        ["禹会区", 117.35],
        ["淮上区", 117.37],
        ["怀远县", 117.21],
        ["五河县", 117.89],
        ["固镇县", 117.32],
        ["蚌埠市高新技术开发区", 117.25],
        ["蚌埠市经济开发区", 117.26],
      ]),
      c("淮南", [
        ["市区", 117.01],
        ["大通区", 117.06],
        ["田家庵区", 117.02],
        ["谢家集区", 116.87],
        ["八公山区", 116.84],
        ["潘集区", 116.84],
        ["凤台县", 116.72],
        ["寿县", 116.81],
      ]),
      c("马鞍山", [
        ["市区", 118.51],
        ["花山区", 118.5],
        ["雨山区", 118.51],
        ["博望区", 118.82],
        ["当涂县", 118.5],
        ["含山县", 118.11],
        ["和县", 118.36],
      ]),
      c("淮北", [
        ["市区", 116.8],
        ["杜集区", 116.83],
        ["相山区", 116.8],
        ["烈山区", 116.82],
        ["濉溪县", 116.77],
      ]),
      c("铜陵", [
        ["市区", 117.82],
        ["铜官区", 117.82],
        ["义安区", 117.8],
        ["郊区", 117.81],
        ["枞阳县", 117.23],
      ]),
      c("安庆", [
        ["市区", 117.06],
        ["迎江区", 117.1],
        ["大观区", 117.04],
        ["宜秀区", 116.99],
        ["怀宁县", 116.84],
        ["太湖县", 116.32],
        ["宿松县", 116.14],
        ["望江县", 116.7],
        ["岳西县", 116.37],
        ["安徽安庆经济开发区", 116.35],
        ["桐城市", 116.98],
        ["潜山市", 116.59],
      ]),
      c("黄山", [
        ["市区", 118.35],
        ["屯溪区", 118.32],
        ["黄山区", 118.15],
        ["徽州区", 118.34],
        ["歙县", 118.42],
        ["休宁县", 118.21],
        ["黟县", 117.94],
        ["祁门县", 117.72],
      ]),
      c("滁州", [
        ["市区", 118.34],
        ["琅琊区", 118.31],
        ["南谯区", 118.42],
        ["来安县", 118.44],
        ["全椒县", 118.28],
        ["定远县", 117.7],
        ["凤阳县", 117.54],
        ["苏滁现代产业园", 118.42],
        ["滁州经济技术开发区", 118.34],
        ["天长市", 119.01],
        ["明光市", 118.0],
      ]),
      c("阜阳", [
        ["市区", 115.82],
        ["颍州区", 115.81],
        ["颍东区", 115.86],
        ["颍泉区", 115.82],
        ["临泉县", 115.27],
        ["太和县", 115.63],
        ["阜南县", 115.6],
        ["颍上县", 116.26],
        ["阜阳合肥现代产业园区", 115.91],
        ["阜阳经济技术开发区", 115.86],
        ["界首市", 115.38],
      ]),
      c("宿州", [
        ["市区", 116.97],
        ["埇桥区", 116.98],
        ["砀山县", 116.37],
        ["萧县", 116.95],
        ["灵璧县", 117.56],
        ["泗县", 117.92],
        ["宿州马鞍山现代产业园区", 117.28],
        ["宿州经济技术开发区", 116.97],
      ]),
      c("六安", [
        ["市区", 116.53],
        ["金安区", 116.55],
        ["裕安区", 116.49],
        ["叶集区", 115.95],
        ["霍邱县", 116.28],
        ["舒城县", 116.96],
        ["金寨县", 115.94],
        ["霍山县", 116.34],
      ]),
      c("亳州", [
        ["市区", 115.78],
        ["谯城区", 115.79],
        ["涡阳县", 116.22],
        ["蒙城县", 116.57],
        ["利辛县", 116.19],
      ]),
      c("池州", [
        ["市区", 117.5],
        ["贵池区", 117.57],
        ["东至县", 117.03],
        ["石台县", 117.49],
        ["青阳县", 117.85],
      ]),
      c("宣城", [
        ["市区", 118.77],
        ["宣州区", 118.76],
        ["郎溪县", 119.19],
        ["泾县", 118.43],
        ["绩溪县", 118.59],
        ["旌德县", 118.55],
        ["宣城市经济开发区", 118.77],
        ["宁国市", 118.99],
        ["广德市", 119.43],
      ]),
    ],
  },
  {
    name: "福建",
    cities: [
      c("福州", [
        ["市区", 119.3],
        ["鼓楼区", 119.31],
        ["台江区", 119.32],
        ["仓山区", 119.28],
        ["马尾区", 119.45],
        ["晋安区", 119.33],
        ["长乐区", 119.53],
        ["闽侯县", 119.14],
        ["连江县", 119.55],
        ["罗源县", 119.56],
        ["闽清县", 118.87],
        ["永泰县", 118.94],
        ["平潭县", 119.8],
        ["福清市", 119.39],
      ]),
      c("厦门", [
        ["市区", 118.1],
        ["思明区", 118.09],
        ["海沧区", 118.04],
        ["湖里区", 118.15],
        ["集美区", 118.1],
        ["同安区", 118.16],
        ["翔安区", 118.25],
      ]),
      c("莆田", [
        ["市区", 119.01],
        ["城厢区", 119.0],
        ["涵江区", 119.12],
        ["荔城区", 119.02],
        ["秀屿区", 119.11],
        ["仙游县", 118.7],
      ]),
      c("三明", [
        ["市区", 117.65],
        ["梅列区", 117.65],
        ["三元区", 117.61],
        ["明溪县", 117.21],
        ["清流县", 116.82],
        ["宁化县", 116.66],
        ["大田县", 117.85],
        ["尤溪县", 118.2],
        ["沙县", 117.8],
        ["将乐县", 117.48],
        ["泰宁县", 117.18],
        ["建宁县", 116.85],
        ["永安市", 117.37],
      ]),
      c("泉州", [
        ["市区", 118.68],
        ["鲤城区", 118.59],
        ["丰泽区", 118.62],
        ["洛江区", 118.68],
        ["泉港区", 118.92],
        ["惠安县", 118.8],
        ["安溪县", 118.19],
        ["永春县", 118.3],
        ["德化县", 118.25],
        ["金门县", 118.33],
        ["石狮市", 118.65],
        ["晋江市", 118.56],
        ["南安市", 118.39],
      ]),
      c("漳州", [
        ["市区", 117.65],
        ["芗城区", 117.66],
        ["龙文区", 117.72],
        ["云霄县", 117.35],
        ["漳浦县", 117.62],
        ["诏安县", 117.18],
        ["长泰县", 117.77],
        ["东山县", 117.44],
        ["南靖县", 117.36],
        ["平和县", 117.32],
        ["华安县", 117.54],
        ["龙海市", 117.82],
      ]),
      c("南平", [
        ["市区", 118.18],
        ["延平区", 118.19],
        ["建阳区", 118.13],
        ["顺昌县", 117.82],
        ["浦城县", 118.55],
        ["光泽县", 117.34],
        ["松溪县", 118.79],
        ["政和县", 118.86],
        ["邵武市", 117.5],
        ["武夷山市", 118.04],
        ["建瓯市", 118.31],
      ]),
      c("龙岩", [
        ["市区", 117.02],
        ["新罗区", 117.04],
        ["永定区", 116.74],
        ["长汀县", 116.36],
        ["上杭县", 116.43],
        ["武平县", 116.11],
        ["连城县", 116.76],
        ["漳平市", 117.43],
      ]),
      c("宁德", [
        ["市区", 119.55],
        ["蕉城区", 119.53],
        ["霞浦县", 120.01],
        ["古田县", 118.75],
        ["屏南县", 118.99],
        ["寿宁县", 119.51],
        ["周宁县", 119.35],
        ["柘荣县", 119.91],
        ["福安市", 119.65],
        ["福鼎市", 120.22],
      ]),
    ],
  },
  {
    name: "江西",
    cities: [
      c("南昌", [
        ["市区", 115.86],
        ["东湖区", 115.91],
        ["西湖区", 115.88],
        ["青云谱区", 115.93],
        ["湾里区", 115.74],
        ["青山湖区", 115.97],
        ["新建区", 115.82],
        ["南昌县", 115.95],
        ["安义县", 115.56],
        ["进贤县", 116.25],
      ]),
      c("景德镇", [
        ["市区", 117.18],
        ["昌江区", 117.19],
        ["珠山区", 117.21],
        ["浮梁县", 117.22],
        ["乐平市", 117.16],
      ]),
      c("萍乡", [
        ["市区", 113.86],
        ["安源区", 113.88],
        ["湘东区", 113.74],
        ["莲花县", 113.97],
        ["上栗县", 113.8],
        ["芦溪县", 114.04],
      ]),
      c("九江", [
        ["市区", 116.01],
        ["濂溪区", 116.0],
        ["浔阳区", 116.0],
        ["柴桑区", 115.92],
        ["武宁县", 115.11],
        ["修水县", 114.55],
        ["永修县", 115.82],
        ["德安县", 115.76],
        ["都昌县", 116.21],
        ["湖口县", 116.26],
        ["彭泽县", 116.56],
        ["瑞昌市", 115.69],
        ["共青城市", 115.82],
        ["庐山市", 116.05],
      ]),
      c("新余", [
        ["市区", 114.92],
        ["渝水区", 114.95],
        ["分宜县", 114.7],
      ]),
      c("鹰潭", [
        ["市区", 117.08],
        ["月湖区", 117.04],
        ["余江区", 116.82],
        ["贵溪市", 117.25],
      ]),
      c("赣州", [
        ["市区", 114.94],
        ["章贡区", 114.93],
        ["南康区", 114.77],
        ["赣县区", 115.02],
        ["信丰县", 114.93],
        ["大余县", 114.37],
        ["上犹县", 114.56],
        ["崇义县", 114.31],
        ["安远县", 115.4],
        ["龙南县", 114.8],
        ["定南县", 115.03],
        ["全南县", 114.54],
        ["宁都县", 116.02],
        ["于都县", 115.42],
        ["兴国县", 115.37],
        ["会昌县", 115.79],
        ["寻乌县", 115.65],
        ["石城县", 116.35],
        ["瑞金市", 116.03],
      ]),
      c("吉安", [
        ["市区", 115.0],
        ["吉州区", 115.0],
        ["青原区", 115.02],
        ["吉安县", 114.91],
        ["吉水县", 115.14],
        ["峡江县", 115.32],
        ["新干县", 115.39],
        ["永丰县", 115.45],
        ["泰和县", 114.92],
        ["遂川县", 114.53],
        ["万安县", 114.79],
        ["安福县", 114.63],
        ["永新县", 114.25],
        ["井冈山市", 114.3],
      ]),
      c("宜春", [
        ["市区", 114.42],
        ["袁州区", 114.43],
        ["奉新县", 115.41],
        ["万载县", 114.45],
        ["上高县", 114.93],
        ["宜丰县", 114.81],
        ["靖安县", 115.37],
        ["铜鼓县", 114.38],
        ["丰城市", 115.78],
        ["樟树市", 115.55],
        ["高安市", 115.38],
      ]),
      c("抚州", [
        ["市区", 116.36],
        ["临川区", 116.32],
        ["东乡区", 116.61],
        ["南城县", 116.64],
        ["黎川县", 116.91],
        ["南丰县", 116.53],
        ["崇仁县", 116.08],
        ["乐安县", 115.84],
        ["宜黄县", 116.24],
        ["金溪县", 116.76],
        ["资溪县", 117.07],
        ["广昌县", 116.34],
      ]),
      c("上饶", [
        ["市区", 117.95],
        ["信州区", 117.97],
        ["广丰区", 118.2],
        ["广信区", 117.91],
        ["玉山县", 118.25],
        ["铅山县", 117.72],
        ["横峰县", 117.6],
        ["弋阳县", 117.46],
        ["余干县", 116.7],
        ["鄱阳县", 116.71],
        ["万年县", 117.06],
        ["婺源县", 117.87],
        ["德兴市", 117.59],
      ]),
    ],
  },
  {
    name: "山东",
    cities: [
      c("济南", [
        ["市区", 117.13],
        ["历下区", 117.08],
        ["市中区", 117.0],
        ["槐荫区", 116.91],
        ["天桥区", 116.99],
        ["历城区", 117.07],
        ["长清区", 116.76],
        ["章丘区", 117.53],
        ["济阳区", 117.18],
        ["莱芜区", 117.67],
        ["钢城区", 117.82],
        ["平阴县", 116.46],
        ["商河县", 117.16],
        ["济南高新技术产业开发区", 117.32],
      ]),
      c("青岛", [
        ["市区", 120.39],
        ["市南区", 120.42],
        ["市北区", 120.38],
        ["黄岛区", 120.2],
        ["崂山区", 120.48],
        ["李沧区", 120.44],
        ["城阳区", 120.4],
        ["即墨区", 120.45],
        ["青岛高新技术产业开发区", 120.54],
        ["胶州市", 120.04],
        ["平度市", 119.99],
        ["莱西市", 120.52],
      ]),
      c("淄博", [
        ["市区", 118.06],
        ["淄川区", 117.97],
        ["张店区", 118.02],
        ["博山区", 117.87],
        ["临淄区", 118.32],
        ["周村区", 117.88],
        ["桓台县", 118.1],
        ["高青县", 117.83],
        ["沂源县", 118.18],
      ]),
      c("枣庄", [
        ["市区", 117.33],
        ["市中区", 117.56],
        ["薛城区", 117.27],
        ["峄城区", 117.6],
        ["台儿庄区", 117.74],
        ["山亭区", 117.47],
        ["滕州市", 117.17],
      ]),
      c("东营", [
        ["市区", 118.68],
        ["东营区", 118.59],
        ["河口区", 118.53],
        ["垦利区", 118.58],
        ["利津县", 118.26],
        ["广饶县", 118.41],
        ["东营经济技术开发区", 118.7],
        ["东营港经济开发区", 118.59],
      ]),
      c("烟台", [
        ["市区", 121.45],
        ["芝罘区", 121.41],
        ["福山区", 121.27],
        ["牟平区", 121.61],
        ["莱山区", 121.45],
        ["长岛县", 120.74],
        ["烟台高新技术产业开发区", 121.53],
        ["烟台经济技术开发区", 121.26],
        ["龙口市", 120.49],
        ["莱阳市", 120.72],
        ["莱州市", 119.95],
        ["蓬莱市", 120.77],
        ["招远市", 120.44],
        ["栖霞市", 120.86],
        ["海阳市", 121.18],
      ]),
      c("潍坊", [
        ["市区", 119.17],
        ["潍城区", 119.03],
        ["寒亭区", 119.23],
        ["坊子区", 119.17],
        ["奎文区", 119.14],
        ["临朐县", 118.55],
        ["昌乐县", 118.84],
        ["潍坊滨海经济技术开发区", 119.19],
        ["青州市", 118.49],
        ["诸城市", 119.42],
        ["寿光市", 118.8],
        ["安丘市", 119.23],
        ["高密市", 119.76],
        ["昌邑市", 119.4],
      ]),
      c("济宁", [
        ["市区", 116.59],
        ["任城区", 116.55],
        ["兖州区", 116.79],
        ["微山县", 117.14],
        ["鱼台县", 116.66],
        ["金乡县", 116.32],
        ["嘉祥县", 116.35],
        ["汶上县", 116.51],
        ["泗水县", 117.26],
        ["梁山县", 116.1],
        ["济宁高新技术产业开发区", 116.63],
        ["曲阜市", 116.99],
        ["邹城市", 117.01],
      ]),
      c("泰安", [
        ["市区", 117.09],
        ["泰山区", 117.14],
        ["岱岳区", 117.05],
        ["宁阳县", 116.81],
        ["东平县", 116.48],
        ["新泰市", 117.77],
        ["肥城市", 116.78],
      ]),
      c("威海", [
        ["市区", 122.13],
        ["环翠区", 122.13],
        ["文登区", 122.06],
        ["威海火炬高技术产业开发区", 122.04],
        ["威海经济技术开发区", 122.18],
        ["威海临港经济技术开发区", 122.13],
        ["荣成市", 122.49],
        ["乳山市", 121.55],
      ]),
      c("日照", [
        ["市区", 119.53],
        ["东港区", 119.47],
        ["岚山区", 119.33],
        ["五莲县", 119.22],
        ["莒县", 118.84],
        ["日照经济技术开发区", 119.53],
      ]),
      c("临沂", [
        ["市区", 118.36],
        ["兰山区", 118.35],
        ["罗庄区", 118.29],
        ["河东区", 118.41],
        ["沂南县", 118.47],
        ["郯城县", 118.37],
        ["沂水县", 118.63],
        ["兰陵县", 118.08],
        ["费县", 117.98],
        ["平邑县", 117.65],
        ["莒南县", 118.84],
        ["蒙阴县", 117.95],
        ["临沭县", 118.66],
        ["临沂高新技术产业开发区", 118.27],
        ["临沂经济技术开发区", 118.42],
        ["临沂临港经济开发区", 118.36],
      ]),
      c("德州", [
        ["市区", 116.37],
        ["德城区", 116.31],
        ["陵城区", 116.59],
        ["宁津县", 116.81],
        ["庆云县", 117.39],
        ["临邑县", 116.87],
        ["齐河县", 116.77],
        ["平原县", 116.44],
        ["夏津县", 116.01],
        ["武城县", 116.08],
        ["德州经济技术开发区", 116.38],
        ["德州运河经济开发区", 116.84],
        ["乐陵市", 117.24],
        ["禹城市", 116.64],
      ]),
      c("聊城", [
        ["市区", 115.99],
        ["东昌府区", 115.99],
        ["茌平区", 116.26],
        ["阳谷县", 115.8],
        ["莘县", 115.68],
        ["东阿县", 116.25],
        ["冠县", 115.45],
        ["高唐县", 116.24],
        ["临清市", 115.71],
      ]),
      c("滨州", [
        ["市区", 117.98],
        ["滨城区", 118.03],
        ["沾化区", 118.11],
        ["惠民县", 117.52],
        ["阳信县", 117.58],
        ["无棣县", 117.63],
        ["博兴县", 118.12],
        ["邹平市", 117.75],
      ]),
      c("菏泽", [
        ["市区", 115.49],
        ["牡丹区", 115.42],
        ["定陶区", 115.58],
        ["曹县", 115.55],
        ["单县", 116.11],
        ["成武县", 115.9],
        ["巨野县", 116.1],
        ["郓城县", 115.95],
        ["鄄城县", 115.52],
        ["东明县", 115.1],
        ["菏泽经济技术开发区", 115.49],
        ["菏泽高新技术开发区", 115.43],
      ]),
    ],
  },
  {
    name: "河南",
    cities: [
      c("郑州", [
        ["市区", 113.63],
        ["中原区", 113.62],
        ["二七区", 113.63],
        ["管城回族区", 113.68],
        ["金水区", 113.67],
        ["上街区", 113.32],
        ["惠济区", 113.62],
        ["中牟县", 113.98],
        ["郑州经济技术开发区", 113.83],
        ["郑州高新技术产业开发区", 114.02],
        ["郑州航空港经济综合实验区", 113.82],
        ["巩义市", 113.03],
        ["荥阳市", 113.39],
        ["新密市", 113.4],
        ["新郑市", 113.75],
        ["登封市", 113.06],
      ]),
      c("开封", [
        ["市区", 114.31],
        ["龙亭区", 114.36],
        ["顺河回族区", 114.37],
        ["鼓楼区", 114.35],
        ["禹王台区", 114.35],
        ["祥符区", 114.45],
        ["杞县", 114.79],
        ["通许县", 114.47],
        ["尉氏县", 114.2],
        ["兰考县", 114.83],
      ]),
      c("洛阳", [
        ["市区", 112.46],
        ["老城区", 112.48],
        ["西工区", 112.43],
        ["瀍河回族区", 112.51],
        ["涧西区", 112.4],
        ["吉利区", 112.6],
        ["洛龙区", 112.47],
        ["孟津县", 112.45],
        ["新安县", 112.14],
        ["栾川县", 111.62],
        ["嵩县", 112.09],
        ["汝阳县", 112.48],
        ["宜阳县", 112.18],
        ["洛宁县", 111.66],
        ["伊川县", 112.43],
        ["洛阳高新技术产业开发区", 112.38],
        ["偃师市", 112.8],
      ]),
      c("平顶山", [
        ["市区", 113.2],
        ["新华区", 113.3],
        ["卫东区", 113.34],
        ["石龙区", 112.91],
        ["湛河区", 113.25],
        ["宝丰县", 113.06],
        ["叶县", 113.36],
        ["鲁山县", 112.91],
        ["郏县", 113.22],
        ["平顶山高新技术产业开发区", 113.39],
        ["平顶山市城乡一体化示范区", 113.2],
        ["舞钢市", 113.53],
        ["汝州市", 112.85],
      ]),
      c("安阳", [
        ["市区", 114.4],
        ["文峰区", 114.36],
        ["北关区", 114.36],
        ["殷都区", 114.31],
        ["龙安区", 114.31],
        ["安阳县", 114.24],
        ["汤阴县", 114.36],
        ["滑县", 114.53],
        ["内黄县", 114.91],
        ["安阳高新技术产业开发区", 114.38],
        ["林州市", 113.83],
      ]),
      c("鹤壁", [
        ["市区", 114.3],
        ["鹤山区", 114.17],
        ["山城区", 114.19],
        ["淇滨区", 114.31],
        ["浚县", 114.56],
        ["淇县", 114.22],
        ["鹤壁经济技术开发区", 114.32],
      ]),
      c("新乡", [
        ["市区", 113.93],
        ["红旗区", 113.88],
        ["卫滨区", 113.87],
        ["凤泉区", 113.91],
        ["牧野区", 113.92],
        ["新乡县", 113.81],
        ["获嘉县", 113.66],
        ["原阳县", 113.95],
        ["延津县", 114.21],
        ["封丘县", 114.43],
        ["新乡高新技术产业开发区", 113.81],
        ["新乡经济技术开发区", 113.81],
        ["新乡市平原城乡一体化示范区", 113.81],
        ["卫辉市", 114.07],
        ["辉县市", 113.81],
        ["长垣市", 114.68],
      ]),
      c("焦作", [
        ["市区", 113.25],
        ["解放区", 113.24],
        ["中站区", 113.19],
        ["马村区", 113.33],
        ["山阳区", 113.26],
        ["修武县", 113.45],
        ["博爱县", 113.07],
        ["武陟县", 113.41],
        ["温县", 113.09],
        ["焦作城乡一体化示范区", 113.26],
        ["沁阳市", 112.96],
        ["孟州市", 112.8],
      ]),
      c("濮阳", [
        ["市区", 115.04],
        ["华龙区", 115.08],
        ["清丰县", 115.11],
        ["南乐县", 115.21],
        ["范县", 115.51],
        ["台前县", 115.88],
        ["濮阳县", 115.04],
        ["河南濮阳工业园区", 115.19],
        ["濮阳经济技术开发区", 115.04],
      ]),
      c("许昌", [
        ["市区", 113.86],
        ["魏都区", 113.84],
        ["建安区", 113.83],
        ["鄢陵县", 114.18],
        ["襄城县", 113.51],
        ["许昌经济技术开发区", 113.81],
        ["禹州市", 113.5],
        ["长葛市", 113.82],
      ]),
      c("漯河", [
        ["市区", 114.02],
        ["源汇区", 113.99],
        ["郾城区", 114.01],
        ["召陵区", 114.1],
        ["舞阳县", 113.62],
        ["临颍县", 113.94],
        ["漯河经济技术开发区", 114.06],
      ]),
      c("三门峡", [
        ["市区", 111.21],
        ["湖滨区", 111.2],
        ["陕州区", 111.11],
        ["渑池县", 111.77],
        ["卢氏县", 111.05],
        ["河南三门峡经济开发区", 111.2],
        ["义马市", 111.88],
        ["灵宝市", 110.9],
      ]),
      c("南阳", [
        ["市区", 112.53],
        ["宛城区", 112.55],
        ["卧龙区", 112.54],
        ["南召县", 112.44],
        ["方城县", 113.02],
        ["西峡县", 111.48],
        ["镇平县", 112.24],
        ["内乡县", 111.86],
        ["淅川县", 111.5],
        ["社旗县", 112.95],
        ["唐河县", 112.81],
        ["新野县", 112.37],
        ["桐柏县", 113.43],
        ["南阳高新技术产业开发区", 112.54],
        ["南阳市城乡一体化示范区", 112.59],
        ["邓州市", 112.09],
      ]),
      c("商丘", [
        ["市区", 115.66],
        ["梁园区", 115.62],
        ["睢阳区", 115.66],
        ["民权县", 115.15],
        ["睢县", 115.08],
        ["宁陵县", 115.32],
        ["柘城县", 115.31],
        ["虞城县", 115.83],
        ["夏邑县", 116.14],
        ["豫东综合物流产业聚集区", 115.82],
        ["河南商丘经济开发区", 115.7],
        ["永城市", 116.46],
      ]),
      c("信阳", [
        ["市区", 114.1],
        ["浉河区", 114.07],
        ["平桥区", 114.13],
        ["罗山县", 114.52],
        ["光山县", 114.93],
        ["新县", 114.89],
        ["商城县", 115.41],
        ["固始县", 115.66],
        ["潢川县", 115.06],
        ["淮滨县", 115.43],
        ["息县", 114.75],
        ["信阳高新技术产业开发区", 114.21],
      ]),
      c("周口", [
        ["市区", 114.7],
        ["川汇区", 114.66],
        ["淮阳区", 114.89],
        ["扶沟县", 114.4],
        ["西华县", 114.54],
        ["商水县", 114.62],
        ["沈丘县", 115.11],
        ["郸城县", 115.18],
        ["太康县", 114.84],
        ["鹿邑县", 115.49],
        ["河南周口经济开发区", 114.68],
        ["项城市", 114.88],
      ]),
      c("驻马店", [
        ["市区", 114.03],
        ["驿城区", 114.0],
        ["西平县", 114.03],
        ["上蔡县", 114.27],
        ["平舆县", 114.63],
        ["正阳县", 114.4],
        ["确山县", 114.03],
        ["泌阳县", 113.33],
        ["汝南县", 114.37],
        ["遂平县", 114.02],
        ["新蔡县", 114.99],
        ["河南驻马店经济开发区", 114.08],
      ]),
      c("河南", [
        ["市区", 113.76],
        ["济源市", 112.59],
      ]),
    ],
  },
  {
    name: "湖北",
    cities: [
      c("武汉", [
        ["市区", 114.31],
        ["江岸区", 114.32],
        ["江汉区", 114.28],
        ["硚口区", 114.22],
        ["汉阳区", 114.23],
        ["武昌区", 114.32],
        ["青山区", 114.39],
        ["洪山区", 114.35],
        ["东西湖区", 114.14],
        ["汉南区", 114.09],
        ["蔡甸区", 114.04],
        ["江夏区", 114.33],
        ["黄陂区", 114.38],
        ["新洲区", 114.81],
      ]),
      c("黄石", [
        ["市区", 115.05],
        ["黄石港区", 115.07],
        ["西塞山区", 115.12],
        ["下陆区", 114.97],
        ["铁山区", 114.91],
        ["阳新县", 115.22],
        ["大冶市", 114.99],
      ]),
      c("十堰", [
        ["市区", 110.8],
        ["茅箭区", 110.82],
        ["张湾区", 110.78],
        ["郧阳区", 110.82],
        ["郧西县", 110.43],
        ["竹山县", 110.24],
        ["竹溪县", 109.72],
        ["房县", 110.75],
        ["丹江口市", 111.52],
      ]),
      c("宜昌", [
        ["市区", 111.29],
        ["西陵区", 111.29],
        ["伍家岗区", 111.37],
        ["点军区", 111.27],
        ["猇亭区", 111.44],
        ["夷陵区", 111.33],
        ["远安县", 111.65],
        ["兴山县", 110.75],
        ["秭归县", 110.98],
        ["长阳土家族自治县", 111.21],
        ["五峰土家族自治县", 111.05],
        ["宜都市", 111.46],
        ["当阳市", 111.79],
        ["枝江市", 111.77],
      ]),
      c("襄阳", [
        ["市区", 112.13],
        ["襄城区", 112.14],
        ["樊城区", 112.14],
        ["襄州区", 112.22],
        ["南漳县", 111.85],
        ["谷城县", 111.66],
        ["保康县", 111.27],
        ["老河口市", 111.68],
        ["枣阳市", 112.78],
        ["宜城市", 112.26],
      ]),
      c("鄂州", [
        ["市区", 114.9],
        ["梁子湖区", 114.69],
        ["华容区", 114.74],
        ["鄂城区", 114.9],
      ]),
      c("荆门", [
        ["市区", 112.21],
        ["东宝区", 112.21],
        ["掇刀区", 112.21],
        ["沙洋县", 112.6],
        ["钟祥市", 112.59],
        ["京山市", 113.13],
      ]),
      c("孝感", [
        ["市区", 113.92],
        ["孝南区", 113.92],
        ["孝昌县", 114.0],
        ["大悟县", 114.13],
        ["云梦县", 113.76],
        ["应城市", 113.58],
        ["安陆市", 113.7],
        ["汉川市", 113.85],
      ]),
      c("荆州", [
        ["市区", 112.25],
        ["沙市区", 112.26],
        ["荆州区", 112.2],
        ["公安县", 112.24],
        ["监利县", 112.91],
        ["江陵县", 112.43],
        ["荆州经济技术开发区", 112.2],
        ["石首市", 112.43],
        ["洪湖市", 113.48],
        ["松滋市", 111.77],
      ]),
      c("黄冈", [
        ["市区", 114.88],
        ["黄州区", 114.89],
        ["团风县", 114.88],
        ["红安县", 114.62],
        ["罗田县", 115.41],
        ["英山县", 115.69],
        ["浠水县", 115.27],
        ["蕲春县", 115.44],
        ["黄梅县", 115.95],
        ["龙感湖管理区", 116.01],
        ["麻城市", 115.02],
        ["武穴市", 115.57],
      ]),
      c("咸宁", [
        ["市区", 114.33],
        ["咸安区", 114.3],
        ["嘉鱼县", 113.95],
        ["通城县", 113.82],
        ["崇阳县", 114.05],
        ["通山县", 114.49],
        ["赤壁市", 113.91],
      ]),
      c("随州", [
        ["市区", 113.39],
        ["曾都区", 113.38],
        ["随县", 113.31],
        ["广水市", 113.83],
      ]),
      c("恩施土家族苗族自治州", [
        ["市区", 109.49],
        ["恩施市", 109.49],
        ["利川市", 108.94],
        ["建始县", 109.73],
        ["巴东县", 110.35],
        ["宣恩县", 109.5],
        ["咸丰县", 109.15],
        ["来凤县", 109.41],
        ["鹤峰县", 110.04],
      ]),
      c("湖北", [
        ["市区", 114.35],
        ["仙桃市", 114.35],
        ["潜江市", 114.35],
        ["天门市", 114.35],
        ["神农架林区", 114.35],
      ]),
    ],
  },
  {
    name: "湖南",
    cities: [
      c("长沙", [
        ["市区", 112.95],
        ["芙蓉区", 113.04],
        ["天心区", 113.0],
        ["岳麓区", 112.94],
        ["开福区", 112.99],
        ["雨花区", 113.04],
        ["望城区", 112.82],
        ["长沙县", 113.09],
        ["浏阳市", 113.65],
        ["宁乡市", 112.56],
      ]),
      c("株洲", [
        ["市区", 113.14],
        ["荷塘区", 113.18],
        ["芦淞区", 113.16],
        ["石峰区", 113.12],
        ["天元区", 113.09],
        ["渌口区", 113.15],
        ["攸县", 113.35],
        ["茶陵县", 113.55],
        ["炎陵县", 113.78],
        ["云龙示范区", 113.15],
        ["醴陵市", 113.5],
      ]),
      c("湘潭", [
        ["市区", 112.95],
        ["雨湖区", 112.91],
        ["岳塘区", 112.98],
        ["湘潭县", 112.96],
        ["湖南湘潭高新技术产业园区", 112.95],
        ["湘潭昭山示范区", 112.96],
        ["湘潭九华示范区", 112.96],
        ["湘乡市", 112.54],
        ["韶山市", 112.53],
      ]),
      c("衡阳", [
        ["市区", 112.58],
        ["珠晖区", 112.63],
        ["雁峰区", 112.62],
        ["石鼓区", 112.6],
        ["蒸湘区", 112.57],
        ["南岳区", 112.75],
        ["衡阳县", 112.38],
        ["衡南县", 112.68],
        ["衡山县", 112.88],
        ["衡东县", 112.96],
        ["祁东县", 112.1],
        ["衡阳综合保税区", 112.38],
        ["湖南衡阳高新技术产业园区", 112.56],
        ["湖南衡阳松木经济开发区", 112.58],
        ["耒阳市", 112.87],
        ["常宁市", 112.41],
      ]),
      c("邵阳", [
        ["市区", 111.47],
        ["双清区", 111.5],
        ["大祥区", 111.45],
        ["北塔区", 111.46],
        ["新邵县", 111.46],
        ["邵阳县", 111.28],
        ["隆回县", 111.04],
        ["洞口县", 110.58],
        ["绥宁县", 110.16],
        ["新宁县", 110.86],
        ["城步苗族自治县", 110.33],
        ["武冈市", 110.64],
        ["邵东市", 111.75],
      ]),
      c("岳阳", [
        ["市区", 113.14],
        ["岳阳楼区", 113.14],
        ["云溪区", 113.28],
        ["君山区", 113.01],
        ["岳阳县", 113.12],
        ["华容县", 112.55],
        ["湘阴县", 112.89],
        ["平江县", 113.59],
        ["岳阳市屈原管理区", 112.91],
        ["汨罗市", 113.07],
        ["临湘市", 113.46],
      ]),
      c("常德", [
        ["市区", 111.71],
        ["武陵区", 111.69],
        ["鼎城区", 111.69],
        ["安乡县", 112.18],
        ["汉寿县", 111.98],
        ["澧县", 111.77],
        ["临澧县", 111.65],
        ["桃源县", 111.5],
        ["石门县", 111.39],
        ["常德市西洞庭管理区", 111.99],
        ["津市市", 111.88],
      ]),
      c("张家界", [
        ["市区", 110.49],
        ["永定区", 110.54],
        ["武陵源区", 110.56],
        ["慈利县", 111.15],
        ["桑植县", 110.17],
      ]),
      c("益阳", [
        ["市区", 112.36],
        ["资阳区", 112.33],
        ["赫山区", 112.38],
        ["南县", 112.4],
        ["桃江县", 112.16],
        ["安化县", 111.22],
        ["益阳市大通湖管理区", 112.54],
        ["湖南益阳高新技术产业园区", 112.36],
        ["沅江市", 112.36],
      ]),
      c("郴州", [
        ["市区", 113.02],
        ["北湖区", 113.02],
        ["苏仙区", 113.05],
        ["桂阳县", 112.74],
        ["宜章县", 112.96],
        ["永兴县", 113.12],
        ["嘉禾县", 112.38],
        ["临武县", 112.57],
        ["汝城县", 113.69],
        ["桂东县", 113.95],
        ["安仁县", 113.28],
        ["资兴市", 113.24],
      ]),
      c("永州", [
        ["市区", 111.62],
        ["零陵区", 111.64],
        ["冷水滩区", 111.6],
        ["祁阳县", 111.85],
        ["东安县", 111.32],
        ["双牌县", 111.67],
        ["道县", 111.61],
        ["江永县", 111.35],
        ["宁远县", 111.95],
        ["蓝山县", 112.2],
        ["新田县", 112.21],
        ["江华瑶族自治县", 111.59],
        ["永州经济技术开发区", 111.62],
        ["永州市金洞管理区", 112.1],
        ["永州市回龙圩管理区", 111.34],
      ]),
      c("怀化", [
        ["市区", 110.01],
        ["鹤城区", 110.05],
        ["中方县", 109.95],
        ["沅陵县", 110.4],
        ["辰溪县", 110.19],
        ["溆浦县", 110.6],
        ["会同县", 109.74],
        ["麻阳苗族自治县", 109.81],
        ["新晃侗族自治县", 109.18],
        ["芷江侗族自治县", 109.69],
        ["靖州苗族侗族自治县", 109.7],
        ["通道侗族自治县", 109.79],
        ["怀化市洪江管理区", 109.84],
        ["洪江市", 109.84],
      ]),
      c("娄底", [
        ["市区", 112.0],
        ["娄星区", 112.01],
        ["双峰县", 112.18],
        ["新化县", 111.33],
        ["冷水江市", 111.44],
        ["涟源市", 111.67],
      ]),
      c("湘西土家族苗族自治州", [
        ["市区", 109.75],
        ["吉首市", 109.7],
        ["泸溪县", 110.23],
        ["凤凰县", 109.61],
        ["花垣县", 109.49],
        ["保靖县", 109.67],
        ["古丈县", 109.96],
        ["永顺县", 109.86],
        ["龙山县", 109.45],
        ["湖南永顺经济开发区", 109.86],
      ]),
    ],
  },
  {
    name: "广东",
    cities: [
      c("广州", [
        ["市区", 113.27],
        ["荔湾区", 113.25],
        ["越秀区", 113.27],
        ["海珠区", 113.32],
        ["天河区", 113.37],
        ["白云区", 113.28],
        ["黄埔区", 113.49],
        ["番禺区", 113.39],
        ["花都区", 113.23],
        ["南沙区", 113.53],
        ["从化区", 113.59],
        ["增城区", 113.82],
      ]),
      c("韶关", [
        ["市区", 113.6],
        ["武江区", 113.59],
        ["浈江区", 113.62],
        ["曲江区", 113.61],
        ["始兴县", 114.07],
        ["仁化县", 113.76],
        ["翁源县", 114.14],
        ["乳源瑶族自治县", 113.28],
        ["新丰县", 114.21],
        ["乐昌市", 113.35],
        ["南雄市", 114.32],
      ]),
      c("深圳", [
        ["市区", 114.06],
        ["罗湖区", 114.14],
        ["福田区", 114.06],
        ["南山区", 113.94],
        ["宝安区", 113.89],
        ["龙岗区", 114.25],
        ["盐田区", 114.24],
        ["龙华区", 114.05],
        ["坪山区", 114.35],
        ["光明区", 113.94],
      ]),
      c("珠海", [
        ["市区", 113.58],
        ["香洲区", 113.55],
        ["斗门区", 113.3],
        ["金湾区", 113.37],
      ]),
      c("汕头", [
        ["市区", 116.69],
        ["龙湖区", 116.72],
        ["金平区", 116.71],
        ["濠江区", 116.73],
        ["潮阳区", 116.61],
        ["潮南区", 116.44],
        ["澄海区", 116.76],
        ["南澳县", 117.03],
      ]),
      c("佛山", [
        ["市区", 113.13],
        ["禅城区", 113.13],
        ["南海区", 113.15],
        ["顺德区", 113.3],
        ["三水区", 112.9],
        ["高明区", 112.9],
      ]),
      c("江门", [
        ["市区", 113.09],
        ["蓬江区", 113.08],
        ["江海区", 113.12],
        ["新会区", 113.04],
        ["台山市", 112.8],
        ["开平市", 112.7],
        ["鹤山市", 112.97],
        ["恩平市", 112.31],
      ]),
      c("湛江", [
        ["市区", 110.37],
        ["赤坎区", 110.37],
        ["霞山区", 110.4],
        ["坡头区", 110.46],
        ["麻章区", 110.34],
        ["遂溪县", 110.26],
        ["徐闻县", 110.18],
        ["廉江市", 110.29],
        ["雷州市", 110.1],
        ["吴川市", 110.79],
      ]),
      c("茂名", [
        ["市区", 110.93],
        ["茂南区", 110.93],
        ["电白区", 111.02],
        ["高州市", 110.86],
        ["化州市", 110.65],
        ["信宜市", 110.95],
      ]),
      c("肇庆", [
        ["市区", 112.47],
        ["端州区", 112.49],
        ["鼎湖区", 112.57],
        ["高要区", 112.46],
        ["广宁县", 112.45],
        ["怀集县", 112.19],
        ["封开县", 111.52],
        ["德庆县", 111.79],
        ["四会市", 112.74],
      ]),
      c("惠州", [
        ["市区", 114.42],
        ["惠城区", 114.39],
        ["惠阳区", 114.46],
        ["博罗县", 114.3],
        ["惠东县", 114.73],
        ["龙门县", 114.26],
      ]),
      c("梅州", [
        ["市区", 116.13],
        ["梅江区", 116.12],
        ["梅县区", 116.09],
        ["大埔县", 116.7],
        ["丰顺县", 116.19],
        ["五华县", 115.78],
        ["平远县", 115.9],
        ["蕉岭县", 116.18],
        ["兴宁市", 115.74],
      ]),
      c("汕尾", [
        ["市区", 115.38],
        ["城区", 115.37],
        ["海丰县", 115.33],
        ["陆河县", 115.67],
        ["陆丰市", 115.66],
      ]),
      c("河源", [
        ["市区", 114.71],
        ["源城区", 114.71],
        ["紫金县", 115.19],
        ["龙川县", 115.27],
        ["连平县", 114.5],
        ["和平县", 114.95],
        ["东源县", 114.75],
      ]),
      c("阳江", [
        ["市区", 111.99],
        ["江城区", 111.96],
        ["阳东区", 112.02],
        ["阳西县", 111.62],
        ["阳春市", 111.8],
      ]),
      c("清远", [
        ["市区", 113.06],
        ["清城区", 113.07],
        ["清新区", 112.99],
        ["佛冈县", 113.54],
        ["阳山县", 112.65],
        ["连山壮族瑶族自治县", 112.1],
        ["连南瑶族自治县", 112.29],
        ["英德市", 113.42],
        ["连州市", 112.38],
      ]),
      c("东莞", [
        ["市区", 113.76],
        ["东莞市", 113.76],
      ]),
      c("中山", [
        ["市区", 113.4],
        ["中山市", 113.4],
      ]),
      c("潮州", [
        ["市区", 116.63],
        ["湘桥区", 116.63],
        ["潮安区", 116.68],
        ["饶平县", 117.01],
      ]),
      c("揭阳", [
        ["市区", 116.38],
        ["榕城区", 116.37],
        ["揭东区", 116.42],
        ["揭西县", 115.85],
        ["惠来县", 116.3],
        ["普宁市", 116.17],
      ]),
      c("云浮", [
        ["市区", 112.05],
        ["云城区", 112.05],
        ["云安区", 112.01],
        ["新兴县", 112.23],
        ["郁南县", 111.54],
        ["罗定市", 111.58],
      ]),
    ],
  },
  {
    name: "广西",
    cities: [
      c("南宁", [
        ["市区", 108.37],
        ["兴宁区", 108.38],
        ["青秀区", 108.5],
        ["江南区", 108.28],
        ["西乡塘区", 108.32],
        ["良庆区", 108.33],
        ["邕宁区", 108.49],
        ["武鸣区", 108.28],
        ["隆安县", 107.7],
        ["马山县", 108.18],
        ["上林县", 108.61],
        ["宾阳县", 108.82],
        ["横县", 109.27],
      ]),
      c("柳州", [
        ["市区", 109.43],
        ["城中区", 109.42],
        ["鱼峰区", 109.45],
        ["柳南区", 109.39],
        ["柳北区", 109.41],
        ["柳江区", 109.33],
        ["柳城县", 109.25],
        ["鹿寨县", 109.76],
        ["融安县", 109.4],
        ["融水苗族自治县", 109.26],
        ["三江侗族自治县", 109.61],
      ]),
      c("桂林", [
        ["市区", 110.2],
        ["秀峰区", 110.27],
        ["叠彩区", 110.31],
        ["象山区", 110.29],
        ["七星区", 110.32],
        ["雁山区", 110.32],
        ["临桂区", 110.22],
        ["阳朔县", 110.5],
        ["灵川县", 110.33],
        ["全州县", 111.08],
        ["兴安县", 110.68],
        ["永福县", 109.99],
        ["灌阳县", 111.17],
        ["龙胜各族自治县", 110.02],
        ["资源县", 110.66],
        ["平乐县", 110.65],
        ["恭城瑶族自治县", 110.83],
        ["荔浦市", 110.4],
      ]),
      c("梧州", [
        ["市区", 111.29],
        ["万秀区", 111.33],
        ["长洲区", 111.28],
        ["龙圩区", 111.25],
        ["苍梧县", 111.56],
        ["藤县", 110.92],
        ["蒙山县", 110.53],
        ["岑溪市", 111.0],
      ]),
      c("北海", [
        ["市区", 109.13],
        ["海城区", 109.12],
        ["银海区", 109.15],
        ["铁山港区", 109.43],
        ["合浦县", 109.21],
      ]),
      c("防城港", [
        ["市区", 108.36],
        ["港口区", 108.39],
        ["防城区", 108.36],
        ["上思县", 107.99],
        ["东兴市", 107.98],
      ]),
      c("钦州", [
        ["市区", 108.66],
        ["钦南区", 108.66],
        ["钦北区", 108.64],
        ["灵山县", 109.3],
        ["浦北县", 109.56],
      ]),
      c("贵港", [
        ["市区", 109.61],
        ["港北区", 109.58],
        ["港南区", 109.61],
        ["覃塘区", 109.46],
        ["平南县", 110.4],
        ["桂平市", 110.09],
      ]),
      c("玉林", [
        ["市区", 110.19],
        ["玉州区", 110.16],
        ["福绵区", 110.09],
        ["容县", 110.56],
        ["陆川县", 110.27],
        ["博白县", 109.98],
        ["兴业县", 109.88],
        ["北流市", 110.36],
      ]),
      c("百色", [
        ["市区", 106.62],
        ["右江区", 106.63],
        ["田阳区", 106.92],
        ["田东县", 107.13],
        ["平果县", 107.6],
        ["德保县", 106.62],
        ["那坡县", 105.84],
        ["凌云县", 106.57],
        ["乐业县", 106.56],
        ["田林县", 106.23],
        ["西林县", 105.1],
        ["隆林各族自治县", 105.35],
        ["靖西市", 106.42],
      ]),
      c("贺州", [
        ["市区", 111.57],
        ["八步区", 111.56],
        ["平桂区", 111.48],
        ["昭平县", 110.82],
        ["钟山县", 111.31],
        ["富川瑶族自治县", 111.28],
      ]),
      c("河池", [
        ["市区", 108.09],
        ["金城江区", 108.04],
        ["宜州区", 108.64],
        ["南丹县", 107.55],
        ["天峨县", 107.18],
        ["凤山县", 107.05],
        ["东兰县", 107.38],
        ["罗城仫佬族自治县", 108.91],
        ["环江毛南族自治县", 108.34],
        ["巴马瑶族自治县", 107.27],
        ["都安瑶族自治县", 108.11],
        ["大化瑶族自治县", 108.0],
      ]),
      c("来宾", [
        ["市区", 109.23],
        ["兴宾区", 109.24],
        ["忻城县", 108.67],
        ["象州县", 109.71],
        ["武宣县", 109.67],
        ["金秀瑶族自治县", 110.2],
        ["合山市", 108.89],
      ]),
      c("崇左", [
        ["市区", 107.37],
        ["江州区", 107.36],
        ["扶绥县", 107.91],
        ["宁明县", 107.08],
        ["龙州县", 106.86],
        ["大新县", 107.21],
        ["天等县", 107.15],
        ["凭祥市", 106.77],
      ]),
    ],
  },
  {
    name: "海南",
    cities: [
      c("海口", [
        ["市区", 110.33],
        ["秀英区", 110.3],
        ["龙华区", 110.34],
        ["琼山区", 110.36],
        ["美兰区", 110.37],
      ]),
      c("三亚", [
        ["市区", 109.52],
        ["海棠区", 109.71],
        ["吉阳区", 109.6],
        ["天涯区", 109.4],
        ["崖州区", 109.15],
      ]),
      c("三沙", [
        ["市区", 112.26],
        ["西沙群岛", 112.16],
        ["南沙群岛", 114.4],
        ["中沙群岛的岛礁及其海域", 114.36],
      ]),
      c("儋州", [
        ["市区", 109.17],
        ["儋州市", 109.59],
      ]),
      c("海南", [
        ["市区", 110.36],
        ["五指山市", 110.36],
        ["琼海市", 110.36],
        ["文昌市", 110.36],
        ["万宁市", 110.36],
        ["东方市", 110.36],
        ["定安县", 110.36],
        ["屯昌县", 110.36],
        ["澄迈县", 110.36],
        ["临高县", 110.36],
        ["白沙黎族自治县", 110.47],
        ["昌江黎族自治县", 110.47],
        ["乐东黎族自治县", 109.18],
        ["陵水黎族自治县", 110.47],
        ["保亭黎族苗族自治县", 110.47],
        ["琼中黎族苗族自治县", 110.47],
      ]),
    ],
  },
  {
    name: "重庆",
    cities: [
      c("重庆", [
        ["市区", 106.56],
        ["万州区", 108.42],
        ["涪陵区", 107.4],
        ["渝中区", 106.58],
        ["大渡口区", 106.49],
        ["江北区", 106.58],
        ["沙坪坝区", 106.46],
        ["九龙坡区", 106.52],
        ["南岸区", 106.67],
        ["北碚区", 106.4],
        ["綦江区", 106.37],
        ["大足区", 106.3],
        ["渝北区", 106.64],
        ["巴南区", 106.55],
        ["黔江区", 108.78],
        ["长寿区", 107.09],
        ["江津区", 106.27],
        ["合川区", 106.28],
        ["永川区", 106.56],
        ["南川区", 107.11],
        ["璧山区", 106.24],
        ["铜梁区", 106.08],
        ["潼南区", 105.85],
        ["荣昌区", 105.6],
        ["开州区", 108.4],
        ["梁平区", 107.78],
        ["武隆区", 107.77],
        ["城口县", 108.67],
        ["丰都县", 107.74],
        ["垫江县", 107.34],
        ["忠县", 108.04],
        ["云阳县", 108.7],
        ["奉节县", 109.47],
        ["巫山县", 109.89],
        ["巫溪县", 109.58],
        ["石柱土家族自治县", 108.12],
        ["秀山土家族苗族自治县", 108.76],
        ["酉阳土家族苗族自治县", 108.76],
        ["彭水苗族土家族自治县", 108.17],
      ]),
    ],
  },
  {
    name: "四川",
    cities: [
      c("成都", [
        ["市区", 104.08],
        ["锦江区", 104.09],
        ["青羊区", 104.07],
        ["金牛区", 104.06],
        ["武侯区", 104.05],
        ["成华区", 104.11],
        ["龙泉驿区", 104.28],
        ["青白江区", 104.26],
        ["新都区", 104.17],
        ["温江区", 103.86],
        ["双流区", 103.93],
        ["郫都区", 103.91],
        ["金堂县", 104.42],
        ["大邑县", 103.53],
        ["蒲江县", 103.51],
        ["新津县", 103.82],
        ["都江堰市", 103.65],
        ["彭州市", 103.97],
        ["邛崃市", 103.47],
        ["崇州市", 103.68],
        ["简阳市", 104.55],
      ]),
      c("自贡", [
        ["市区", 104.78],
        ["自流井区", 104.78],
        ["贡井区", 104.72],
        ["大安区", 104.78],
        ["沿滩区", 104.88],
        ["荣县", 104.42],
        ["富顺县", 104.98],
      ]),
      c("攀枝花", [
        ["市区", 101.73],
        ["东区", 101.73],
        ["西区", 101.73],
        ["仁和区", 101.74],
        ["米易县", 102.12],
        ["盐边县", 101.86],
      ]),
      c("泸州", [
        ["市区", 105.45],
        ["江阳区", 105.44],
        ["纳溪区", 105.38],
        ["龙马潭区", 105.44],
        ["泸县", 105.39],
        ["合江县", 105.84],
        ["叙永县", 105.45],
        ["古蔺县", 105.82],
      ]),
      c("德阳", [
        ["市区", 104.4],
        ["旌阳区", 104.42],
        ["罗江区", 104.52],
        ["中江县", 104.69],
        ["广汉市", 104.29],
        ["什邡市", 104.17],
        ["绵竹市", 104.23],
      ]),
      c("绵阳", [
        ["市区", 104.69],
        ["涪城区", 104.76],
        ["游仙区", 104.77],
        ["安州区", 104.57],
        ["三台县", 105.1],
        ["盐亭县", 105.4],
        ["梓潼县", 105.18],
        ["北川羌族自治县", 104.36],
        ["平武县", 104.54],
        ["江油市", 104.75],
      ]),
      c("广元", [
        ["市区", 105.85],
        ["利州区", 105.85],
        ["昭化区", 105.97],
        ["朝天区", 105.9],
        ["旺苍县", 106.3],
        ["青川县", 105.25],
        ["剑阁县", 105.53],
        ["苍溪县", 105.94],
      ]),
      c("遂宁", [
        ["市区", 105.6],
        ["船山区", 105.57],
        ["安居区", 105.46],
        ["蓬溪县", 105.71],
        ["大英县", 105.24],
        ["射洪市", 105.39],
      ]),
      c("内江", [
        ["市区", 105.06],
        ["市中区", 105.07],
        ["东兴区", 105.08],
        ["威远县", 104.67],
        ["资中县", 104.86],
        ["内江经济开发区", 105.02],
        ["隆昌市", 105.29],
      ]),
      c("乐山", [
        ["市区", 103.77],
        ["市中区", 103.77],
        ["沙湾区", 103.56],
        ["五通桥区", 103.82],
        ["金口河区", 103.09],
        ["犍为县", 103.96],
        ["井研县", 104.08],
        ["夹江县", 103.58],
        ["沐川县", 103.91],
        ["峨边彝族自治县", 103.27],
        ["马边彝族自治县", 103.55],
        ["峨眉山市", 103.49],
      ]),
      c("南充", [
        ["市区", 106.12],
        ["顺庆区", 106.1],
        ["高坪区", 106.13],
        ["嘉陵区", 106.08],
        ["南部县", 106.04],
        ["营山县", 106.57],
        ["蓬安县", 106.42],
        ["仪陇县", 106.31],
        ["西充县", 105.91],
        ["阆中市", 106.01],
      ]),
      c("眉山", [
        ["市区", 103.86],
        ["东坡区", 103.84],
        ["彭山区", 103.88],
        ["仁寿县", 104.14],
        ["洪雅县", 103.38],
        ["丹棱县", 103.52],
        ["青神县", 103.85],
      ]),
      c("宜宾", [
        ["市区", 104.65],
        ["翠屏区", 104.63],
        ["南溪区", 104.98],
        ["叙州区", 104.54],
        ["江安县", 105.07],
        ["长宁县", 104.93],
        ["高县", 104.52],
        ["珙县", 104.72],
        ["筠连县", 104.52],
        ["兴文县", 105.24],
        ["屏山县", 104.35],
      ]),
      c("广安", [
        ["市区", 106.64],
        ["广安区", 106.65],
        ["前锋区", 106.9],
        ["岳池县", 106.45],
        ["武胜县", 106.3],
        ["邻水县", 106.94],
        ["华蓥市", 106.79],
      ]),
      c("达州", [
        ["市区", 107.47],
        ["通川区", 107.51],
        ["达川区", 107.52],
        ["宣汉县", 107.73],
        ["开江县", 107.88],
        ["大竹县", 107.21],
        ["渠县", 106.98],
        ["达州经济开发区", 107.5],
        ["万源市", 108.04],
      ]),
      c("雅安", [
        ["市区", 103.05],
        ["雨城区", 103.04],
        ["名山区", 103.12],
        ["荥经县", 102.85],
        ["汉源县", 102.66],
        ["石棉县", 102.37],
        ["天全县", 102.76],
        ["芦山县", 102.93],
        ["宝兴县", 102.82],
      ]),
      c("巴中", [
        ["市区", 106.75],
        ["巴州区", 106.78],
        ["恩阳区", 106.64],
        ["通江县", 107.25],
        ["南江县", 106.83],
        ["平昌县", 107.11],
        ["巴中经济开发区", 106.86],
      ]),
      c("资阳", [
        ["市区", 104.63],
        ["雁江区", 104.66],
        ["安岳县", 105.34],
        ["乐至县", 105.03],
      ]),
      c("阿坝藏族羌族自治州", [
        ["市区", 101.71],
        ["马尔康市", 102.21],
        ["汶川县", 103.6],
        ["理县", 103.17],
        ["茂县", 103.86],
        ["松潘县", 103.61],
        ["九寨沟县", 104.25],
        ["金川县", 102.07],
        ["小金县", 102.37],
        ["黑水县", 103.0],
        ["壤塘县", 100.99],
        ["阿坝县", 101.71],
        ["若尔盖县", 102.97],
        ["红原县", 102.55],
      ]),
      c("甘孜藏族自治州", [
        ["市区", 101.97],
        ["康定市", 101.96],
        ["泸定县", 102.24],
        ["丹巴县", 101.9],
        ["九龙县", 101.51],
        ["雅江县", 101.02],
        ["道孚县", 101.13],
        ["炉霍县", 100.68],
        ["甘孜县", 100.0],
        ["新龙县", 100.32],
        ["德格县", 98.59],
        ["白玉县", 98.83],
        ["石渠县", 98.11],
        ["色达县", 100.34],
        ["理塘县", 100.28],
        ["巴塘县", 99.12],
        ["乡城县", 99.81],
        ["稻城县", 100.3],
        ["得荣县", 99.29],
      ]),
      c("凉山彝族自治州", [
        ["市区", 102.27],
        ["西昌市", 102.27],
        ["木里藏族自治县", 101.29],
        ["盐源县", 101.52],
        ["德昌县", 102.18],
        ["会理县", 102.25],
        ["会东县", 102.58],
        ["宁南县", 102.76],
        ["普格县", 102.55],
        ["布拖县", 102.82],
        ["金阳县", 103.25],
        ["昭觉县", 102.85],
        ["喜德县", 102.42],
        ["冕宁县", 102.18],
        ["越西县", 102.51],
        ["甘洛县", 102.78],
        ["美姑县", 103.14],
        ["雷波县", 103.58],
      ]),
    ],
  },
  {
    name: "贵州",
    cities: [
      c("贵阳", [
        ["市区", 106.64],
        ["南明区", 106.72],
        ["云岩区", 106.73],
        ["花溪区", 106.68],
        ["乌当区", 106.76],
        ["白云区", 106.63],
        ["观山湖区", 106.61],
        ["开阳县", 106.97],
        ["息烽县", 106.75],
        ["修文县", 106.6],
        ["清镇市", 106.48],
      ]),
      c("六盘水", [
        ["市区", 104.84],
        ["钟山区", 104.85],
        ["六枝特区", 105.48],
        ["水城县", 104.96],
        ["盘州市", 104.48],
      ]),
      c("遵义", [
        ["市区", 106.93],
        ["红花岗区", 106.9],
        ["汇川区", 106.94],
        ["播州区", 106.84],
        ["桐梓县", 106.83],
        ["绥阳县", 107.2],
        ["正安县", 107.46],
        ["道真仡佬族苗族自治县", 107.62],
        ["务川仡佬族苗族自治县", 107.91],
        ["凤冈县", 107.72],
        ["湄潭县", 107.47],
        ["余庆县", 107.91],
        ["习水县", 106.2],
        ["赤水市", 105.7],
        ["仁怀市", 106.41],
      ]),
      c("安顺", [
        ["市区", 105.95],
        ["西秀区", 105.97],
        ["平坝区", 106.26],
        ["普定县", 105.75],
        ["镇宁布依族苗族自治县", 105.78],
        ["关岭布依族苗族自治县", 105.63],
        ["紫云苗族布依族自治县", 106.09],
      ]),
      c("毕节", [
        ["市区", 105.3],
        ["七星关区", 105.31],
        ["大方县", 105.62],
        ["黔西县", 106.04],
        ["金沙县", 106.23],
        ["织金县", 105.78],
        ["纳雍县", 105.39],
        ["威宁彝族回族苗族自治县", 104.26],
        ["赫章县", 104.73],
      ]),
      c("铜仁", [
        ["市区", 109.2],
        ["碧江区", 109.19],
        ["万山区", 109.22],
        ["江口县", 108.85],
        ["玉屏侗族自治县", 108.92],
        ["石阡县", 108.23],
        ["思南县", 108.26],
        ["印江土家族苗族自治县", 108.42],
        ["德江县", 108.13],
        ["沿河土家族自治县", 108.51],
        ["松桃苗族自治县", 109.21],
      ]),
      c("黔西南布依族苗族自治州", [
        ["市区", 106.04],
        ["兴义市", 104.9],
        ["兴仁市", 105.19],
        ["普安县", 104.96],
        ["晴隆县", 105.23],
        ["贞丰县", 105.66],
        ["望谟县", 106.11],
        ["册亨县", 105.82],
        ["安龙县", 105.45],
      ]),
      c("黔东南苗族侗族自治州", [
        ["市区", 107.99],
        ["凯里市", 107.99],
        ["黄平县", 107.92],
        ["施秉县", 108.13],
        ["三穗县", 108.68],
        ["镇远县", 108.44],
        ["岑巩县", 108.82],
        ["天柱县", 109.21],
        ["锦屏县", 109.21],
        ["剑河县", 108.45],
        ["台江县", 108.33],
        ["黎平县", 109.14],
        ["榕江县", 108.53],
        ["从江县", 108.91],
        ["雷山县", 108.08],
        ["麻江县", 107.6],
        ["丹寨县", 107.8],
      ]),
      c("黔南布依族苗族自治州", [
        ["市区", 107.53],
        ["都匀市", 107.53],
        ["福泉市", 107.53],
        ["荔波县", 107.89],
        ["贵定县", 107.24],
        ["瓮安县", 107.48],
        ["独山县", 107.55],
        ["平塘县", 107.33],
        ["罗甸县", 106.76],
        ["长顺县", 106.46],
        ["龙里县", 106.99],
        ["惠水县", 106.66],
        ["三都水族自治县", 107.88],
      ]),
    ],
  },
  {
    name: "云南",
    cities: [
      c("昆明", [
        ["市区", 102.85],
        ["五华区", 102.71],
        ["盘龙区", 102.76],
        ["官渡区", 102.75],
        ["西山区", 102.67],
        ["东川区", 103.19],
        ["呈贡区", 102.83],
        ["晋宁区", 102.6],
        ["富民县", 102.5],
        ["宜良县", 103.15],
        ["石林彝族自治县", 103.3],
        ["嵩明县", 103.04],
        ["禄劝彝族苗族自治县", 102.48],
        ["寻甸回族彝族自治县", 103.26],
        ["安宁市", 102.48],
      ]),
      c("曲靖", [
        ["市区", 103.8],
        ["麒麟区", 103.81],
        ["沾益区", 103.83],
        ["马龙区", 103.59],
        ["陆良县", 103.67],
        ["师宗县", 103.99],
        ["罗平县", 104.32],
        ["富源县", 104.26],
        ["会泽县", 103.3],
        ["宣威市", 104.11],
      ]),
      c("玉溪", [
        ["市区", 102.55],
        ["红塔区", 102.55],
        ["江川区", 102.76],
        ["澄江县", 102.91],
        ["通海县", 102.73],
        ["华宁县", 102.94],
        ["易门县", 102.17],
        ["峨山彝族自治县", 102.41],
        ["新平彝族傣族自治县", 102.0],
        ["元江哈尼族彝族傣族自治县", 102.0],
      ]),
      c("保山", [
        ["市区", 99.18],
        ["隆阳区", 99.17],
        ["施甸县", 99.2],
        ["龙陵县", 98.7],
        ["昌宁县", 99.61],
        ["腾冲市", 98.5],
      ]),
      c("昭通", [
        ["市区", 103.72],
        ["昭阳区", 103.71],
        ["鲁甸县", 103.56],
        ["巧家县", 102.94],
        ["盐津县", 104.24],
        ["大关县", 103.9],
        ["永善县", 103.64],
        ["绥江县", 103.96],
        ["镇雄县", 104.88],
        ["彝良县", 104.05],
        ["威信县", 105.06],
        ["水富市", 104.42],
      ]),
      c("丽江", [
        ["市区", 100.23],
        ["古城区", 100.23],
        ["玉龙纳西族自治县", 100.24],
        ["永胜县", 100.76],
        ["华坪县", 101.27],
        ["宁蒗彝族自治县", 100.86],
      ]),
      c("普洱", [
        ["市区", 100.97],
        ["思茅区", 100.98],
        ["宁洱哈尼族彝族自治县", 101.05],
        ["墨江哈尼族自治县", 101.7],
        ["景东彝族自治县", 100.84],
        ["景谷傣族彝族自治县", 100.71],
        ["镇沅彝族哈尼族拉祜族自治县", 101.12],
        ["江城哈尼族彝族自治县", 101.87],
        ["孟连傣族拉祜族佤族自治县", 99.59],
        ["澜沧拉祜族自治县", 99.94],
        ["西盟佤族自治县", 99.6],
      ]),
      c("临沧", [
        ["市区", 100.1],
        ["临翔区", 100.09],
        ["凤庆县", 99.93],
        ["云县", 100.13],
        ["永德县", 99.27],
        ["镇康县", 98.83],
        ["双江拉祜族佤族布朗族傣族自治县", 99.83],
        ["耿马傣族佤族自治县", 99.4],
        ["沧源佤族自治县", 99.25],
      ]),
      c("楚雄彝族自治州", [
        ["市区", 101.53],
        ["楚雄市", 101.55],
        ["双柏县", 101.65],
        ["牟定县", 101.55],
        ["南华县", 101.28],
        ["姚安县", 101.25],
        ["大姚县", 101.33],
        ["永仁县", 101.67],
        ["元谋县", 101.88],
        ["武定县", 102.41],
        ["禄丰县", 102.09],
      ]),
      c("红河哈尼族彝族自治州", [
        ["市区", 102.43],
        ["个旧市", 103.17],
        ["开远市", 103.27],
        ["蒙自市", 103.37],
        ["弥勒市", 103.42],
        ["屏边苗族自治县", 103.69],
        ["建水县", 102.83],
        ["石屏县", 102.5],
        ["泸西县", 103.77],
        ["元阳县", 102.84],
        ["红河县", 102.43],
        ["金平苗族瑶族傣族自治县", 103.23],
        ["绿春县", 102.4],
        ["河口瑶族自治县", 103.95],
      ]),
      c("文山壮族苗族自治州", [
        ["市区", 104.22],
        ["文山市", 104.24],
        ["砚山县", 104.34],
        ["西畴县", 104.68],
        ["麻栗坡县", 104.71],
        ["马关县", 104.4],
        ["丘北县", 104.17],
        ["广南县", 105.06],
        ["富宁县", 105.64],
      ]),
      c("西双版纳傣族自治州", [
        ["市区", 100.8],
        ["景洪市", 100.81],
        ["勐海县", 100.46],
        ["勐腊县", 101.57],
      ]),
      c("大理白族自治州", [
        ["市区", 100.24],
        ["大理市", 100.24],
        ["漾濞彝族自治县", 99.96],
        ["祥云县", 100.56],
        ["宾川县", 100.58],
        ["弥渡县", 100.5],
        ["南涧彝族自治县", 100.52],
        ["巍山彝族回族自治县", 100.31],
        ["永平县", 99.55],
        ["云龙县", 99.38],
        ["洱源县", 99.97],
        ["剑川县", 99.91],
        ["鹤庆县", 100.18],
      ]),
      c("德宏傣族景颇族自治州", [
        ["市区", 98.59],
        ["瑞丽市", 98.59],
        ["芒市", 98.59],
        ["梁河县", 98.3],
        ["盈江县", 97.94],
        ["陇川县", 97.8],
      ]),
      c("怒江傈僳族自治州", [
        ["市区", 98.86],
        ["泸水市", 98.86],
        ["福贡县", 98.88],
        ["贡山独龙族怒族自治县", 98.74],
        ["兰坪白族普米族自治县", 99.42],
      ]),
      c("迪庆藏族自治州", [
        ["市区", 99.71],
        ["香格里拉市", 99.71],
        ["德钦县", 98.92],
        ["维西傈僳族自治县", 99.29],
      ]),
    ],
  },
  {
    name: "西藏",
    cities: [
      c("拉萨", [
        ["市区", 91.12],
        ["城关区", 91.15],
        ["堆龙德庆区", 91.01],
        ["达孜区", 91.36],
        ["林周县", 91.27],
        ["当雄县", 91.11],
        ["尼木县", 90.17],
        ["曲水县", 90.75],
        ["墨竹工卡县", 91.74],
        ["格尔木藏青工业园区", 94.93],
        ["拉萨经济技术开发区", 91.03],
        ["西藏文化旅游创意园区", 91.15],
        ["达孜工业园区", 91.34],
      ]),
      c("日喀则", [
        ["市区", 88.89],
        ["桑珠孜区", 88.9],
        ["南木林县", 89.11],
        ["江孜县", 89.61],
        ["定日县", 87.13],
        ["萨迦县", 88.03],
        ["拉孜县", 87.64],
        ["昂仁县", 87.24],
        ["谢通门县", 88.27],
        ["白朗县", 89.27],
        ["仁布县", 89.85],
        ["康马县", 89.69],
        ["定结县", 87.77],
        ["仲巴县", 84.04],
        ["亚东县", 88.91],
        ["吉隆县", 85.3],
        ["聂拉木县", 85.99],
        ["萨嘎县", 85.24],
        ["岗巴县", 88.53],
      ]),
      c("昌都", [
        ["市区", 97.18],
        ["卡若区", 97.19],
        ["江达县", 98.22],
        ["贡觉县", 98.28],
        ["类乌齐县", 96.61],
        ["丁青县", 95.6],
        ["察雅县", 97.58],
        ["八宿县", 96.92],
        ["左贡县", 97.85],
        ["芒康县", 98.6],
        ["洛隆县", 95.83],
        ["边坝县", 94.71],
      ]),
      c("林芝", [
        ["市区", 94.37],
        ["巴宜区", 94.37],
        ["工布江达县", 93.25],
        ["米林县", 94.22],
        ["墨脱县", 95.34],
        ["波密县", 95.77],
        ["察隅县", 97.47],
        ["朗县", 93.08],
      ]),
      c("山南", [
        ["市区", 91.78],
        ["乃东区", 91.78],
        ["扎囊县", 91.34],
        ["贡嘎县", 90.99],
        ["桑日县", 92.02],
        ["琼结县", 91.69],
        ["曲松县", 92.21],
        ["措美县", 91.44],
        ["洛扎县", 90.87],
        ["加查县", 92.6],
        ["隆子县", 92.47],
        ["错那县", 91.96],
        ["浪卡子县", 90.4],
      ]),
      c("那曲", [
        ["市区", 92.06],
        ["色尼区", 92.06],
        ["嘉黎县", 93.24],
        ["比如县", 93.69],
        ["聂荣县", 92.31],
        ["安多县", 91.69],
        ["申扎县", 88.72],
        ["索县", 93.79],
        ["班戈县", 90.02],
        ["巴青县", 94.06],
        ["尼玛县", 87.24],
        ["双湖县", 88.84],
      ]),
      c("阿里地区", [
        ["市区", 80.11],
        ["普兰县", 81.18],
        ["札达县", 79.81],
        ["噶尔县", 80.1],
        ["日土县", 79.74],
        ["革吉县", 81.15],
        ["改则县", 84.07],
        ["措勤县", 85.17],
      ]),
    ],
  },
  {
    name: "陕西",
    cities: [
      c("西安", [
        ["市区", 108.95],
        ["新城区", 108.97],
        ["碑林区", 108.94],
        ["莲湖区", 108.95],
        ["灞桥区", 109.07],
        ["未央区", 108.95],
        ["雁塔区", 108.95],
        ["阎良区", 109.23],
        ["临潼区", 109.22],
        ["长安区", 108.91],
        ["高陵区", 109.09],
        ["鄠邑区", 108.61],
        ["蓝田县", 109.33],
        ["周至县", 108.23],
      ]),
      c("铜川", [
        ["市区", 108.95],
        ["王益区", 109.08],
        ["印台区", 109.11],
        ["耀州区", 108.99],
        ["宜君县", 109.12],
      ]),
      c("宝鸡", [
        ["市区", 107.24],
        ["渭滨区", 107.16],
        ["金台区", 107.15],
        ["陈仓区", 107.39],
        ["凤翔县", 107.41],
        ["岐山县", 107.63],
        ["扶风县", 107.91],
        ["眉县", 107.76],
        ["陇县", 106.87],
        ["千阳县", 107.14],
        ["麟游县", 107.8],
        ["凤县", 106.52],
        ["太白县", 107.33],
      ]),
      c("咸阳", [
        ["市区", 108.72],
        ["秦都区", 108.71],
        ["杨陵区", 108.09],
        ["渭城区", 108.74],
        ["三原县", 108.95],
        ["泾阳县", 108.85],
        ["乾县", 108.25],
        ["礼泉县", 108.43],
        ["永寿县", 108.15],
        ["长武县", 107.81],
        ["旬邑县", 108.34],
        ["淳化县", 108.59],
        ["武功县", 108.21],
        ["兴平市", 108.5],
        ["彬州市", 108.08],
      ]),
      c("渭南", [
        ["市区", 109.52],
        ["临渭区", 109.5],
        ["华州区", 109.78],
        ["潼关县", 110.25],
        ["大荔县", 109.95],
        ["合阳县", 110.16],
        ["澄城县", 109.94],
        ["蒲城县", 109.59],
        ["白水县", 109.6],
        ["富平县", 109.19],
        ["韩城市", 110.45],
        ["华阴市", 110.1],
      ]),
      c("延安", [
        ["市区", 109.5],
        ["宝塔区", 109.5],
        ["安塞区", 109.34],
        ["延长县", 110.02],
        ["延川县", 110.2],
        ["志丹县", 108.78],
        ["吴起县", 108.18],
        ["甘泉县", 109.36],
        ["富县", 109.39],
        ["洛川县", 109.44],
        ["宜川县", 110.18],
        ["黄龙县", 109.85],
        ["黄陵县", 109.27],
        ["子长市", 109.68],
      ]),
      c("汉中", [
        ["市区", 107.03],
        ["汉台区", 107.04],
        ["南郑区", 106.94],
        ["城固县", 107.34],
        ["洋县", 107.55],
        ["西乡县", 107.77],
        ["勉县", 106.68],
        ["宁强县", 106.26],
        ["略阳县", 106.16],
        ["镇巴县", 107.9],
        ["留坝县", 106.93],
        ["佛坪县", 108.0],
      ]),
      c("榆林", [
        ["市区", 109.74],
        ["榆阳区", 109.73],
        ["横山区", 109.3],
        ["府谷县", 111.07],
        ["靖边县", 108.8],
        ["定边县", 107.61],
        ["绥德县", 110.27],
        ["米脂县", 110.19],
        ["佳县", 110.5],
        ["吴堡县", 110.75],
        ["清涧县", 110.13],
        ["子洲县", 110.04],
        ["神木市", 110.51],
      ]),
      c("安康", [
        ["市区", 109.04],
        ["汉滨区", 109.03],
        ["汉阴县", 108.52],
        ["石泉县", 108.25],
        ["宁陕县", 108.32],
        ["紫阳县", 108.54],
        ["岚皋县", 108.91],
        ["平利县", 109.37],
        ["镇坪县", 109.53],
        ["旬阳县", 109.37],
        ["白河县", 110.12],
      ]),
      c("商洛", [
        ["市区", 109.92],
        ["商州区", 109.95],
        ["洛南县", 110.15],
        ["丹凤县", 110.33],
        ["商南县", 110.89],
        ["山阳县", 109.89],
        ["镇安县", 109.16],
        ["柞水县", 109.12],
      ]),
    ],
  },
  {
    name: "甘肃",
    cities: [
      c("兰州", [
        ["市区", 103.84],
        ["城关区", 103.83],
        ["七里河区", 103.79],
        ["西固区", 103.63],
        ["安宁区", 103.73],
        ["红古区", 102.87],
        ["永登县", 103.27],
        ["皋兰县", 103.95],
        ["榆中县", 104.12],
        ["兰州新区", 103.66],
      ]),
      c("嘉峪关", [
        ["市区", 98.3],
        ["嘉峪关市", 98.3],
      ]),
      c("金昌", [
        ["市区", 102.19],
        ["金川区", 102.2],
        ["永昌县", 101.98],
      ]),
      c("白银", [
        ["市区", 104.14],
        ["白银区", 104.16],
        ["平川区", 104.83],
        ["靖远县", 104.68],
        ["会宁县", 105.06],
        ["景泰县", 104.07],
      ]),
      c("天水", [
        ["市区", 105.73],
        ["秦州区", 105.73],
        ["麦积区", 105.9],
        ["清水县", 106.14],
        ["秦安县", 105.68],
        ["甘谷县", 105.34],
        ["武山县", 104.9],
        ["张家川回族自治县", 106.21],
      ]),
      c("武威", [
        ["市区", 102.64],
        ["凉州区", 102.65],
        ["民勤县", 103.1],
        ["古浪县", 102.9],
        ["天祝藏族自治县", 103.15],
      ]),
      c("张掖", [
        ["市区", 100.46],
        ["甘州区", 100.48],
        ["肃南裕固族自治县", 99.62],
        ["民乐县", 100.82],
        ["临泽县", 100.17],
        ["高台县", 99.83],
        ["山丹县", 101.1],
      ]),
      c("平凉", [
        ["市区", 106.67],
        ["崆峒区", 106.68],
        ["泾川县", 107.37],
        ["灵台县", 107.63],
        ["崇信县", 107.04],
        ["庄浪县", 106.04],
        ["静宁县", 105.74],
        ["华亭市", 106.66],
      ]),
      c("酒泉", [
        ["市区", 98.5],
        ["肃州区", 98.51],
        ["金塔县", 98.91],
        ["瓜州县", 95.79],
        ["肃北蒙古族自治县", 94.88],
        ["阿克塞哈萨克族自治县", 94.35],
        ["玉门市", 97.05],
        ["敦煌市", 94.67],
      ]),
      c("庆阳", [
        ["市区", 107.65],
        ["西峰区", 107.66],
        ["庆城县", 107.89],
        ["环县", 107.31],
        ["华池县", 108.0],
        ["合水县", 108.03],
        ["正宁县", 108.37],
        ["宁县", 107.93],
        ["镇原县", 107.21],
      ]),
      c("定西", [
        ["市区", 104.63],
        ["安定区", 104.62],
        ["通渭县", 105.25],
        ["陇西县", 104.64],
        ["渭源县", 104.22],
        ["临洮县", 103.87],
        ["漳县", 104.48],
        ["岷县", 104.04],
      ]),
      c("陇南", [
        ["市区", 104.93],
        ["武都区", 104.93],
        ["成县", 105.75],
        ["文县", 104.69],
        ["宕昌县", 104.4],
        ["康县", 105.62],
        ["西和县", 105.31],
        ["礼县", 105.18],
        ["徽县", 106.09],
        ["两当县", 106.31],
      ]),
      c("临夏回族自治州", [
        ["市区", 103.25],
        ["临夏市", 103.25],
        ["临夏县", 103.25],
        ["康乐县", 103.72],
        ["永靖县", 103.29],
        ["广河县", 103.58],
        ["和政县", 103.36],
        ["东乡族自治县", 116.61],
        ["积石山保安族东乡族撒拉族自治县", 103.06],
      ]),
      c("甘南藏族自治州", [
        ["市区", 102.92],
        ["合作市", 102.92],
        ["临潭县", 103.36],
        ["卓尼县", 103.51],
        ["舟曲县", 104.38],
        ["迭部县", 103.23],
        ["玛曲县", 102.08],
        ["碌曲县", 102.49],
        ["夏河县", 102.53],
      ]),
    ],
  },
  {
    name: "青海",
    cities: [
      c("西宁", [
        ["市区", 101.78],
        ["城东区", 101.81],
        ["城中区", 101.79],
        ["城西区", 101.77],
        ["城北区", 101.77],
        ["大通回族土族自治县", 101.69],
        ["湟中县", 101.58],
        ["湟源县", 101.26],
      ]),
      c("海东", [
        ["市区", 102.11],
        ["乐都区", 102.41],
        ["平安区", 102.11],
        ["民和回族土族自治县", 102.84],
        ["互助土族自治县", 101.96],
        ["化隆回族自治县", 102.27],
        ["循化撒拉族自治县", 102.5],
      ]),
      c("海北藏族自治州", [
        ["市区", 100.91],
        ["门源回族自治县", 101.63],
        ["祁连县", 100.26],
        ["海晏县", 101.0],
        ["刚察县", 100.15],
      ]),
      c("黄南藏族自治州", [
        ["市区", 102.02],
        ["同仁县", 102.03],
        ["尖扎县", 102.04],
        ["泽库县", 101.47],
        ["河南蒙古族自治县", 101.62],
      ]),
      c("海南藏族自治州", [
        ["市区", 100.63],
        ["共和县", 100.63],
        ["同德县", 100.59],
        ["贵德县", 101.44],
        ["兴海县", 99.99],
        ["贵南县", 100.75],
      ]),
      c("果洛藏族自治州", [
        ["市区", 100.25],
        ["玛沁县", 100.25],
        ["班玛县", 100.74],
        ["甘德县", 99.91],
        ["达日县", 99.66],
        ["久治县", 101.49],
        ["玛多县", 98.22],
      ]),
      c("玉树藏族自治州", [
        ["市区", 97.01],
        ["玉树市", 97.02],
        ["杂多县", 95.31],
        ["称多县", 97.12],
        ["治多县", 95.62],
        ["囊谦县", 96.49],
        ["曲麻莱县", 95.8],
      ]),
      c("海西蒙古族藏族自治州", [
        ["市区", 97.38],
        ["格尔木市", 94.93],
        ["德令哈市", 97.37],
        ["茫崖市", 90.88],
        ["乌兰县", 98.49],
        ["都兰县", 98.1],
        ["天峻县", 99.03],
        ["大柴旦行政委员会", 95.37],
      ]),
    ],
  },
  {
    name: "宁夏",
    cities: [
      c("银川", [
        ["市区", 106.24],
        ["兴庆区", 106.3],
        ["西夏区", 106.16],
        ["金凤区", 106.25],
        ["永宁县", 106.26],
        ["贺兰县", 106.36],
        ["灵武市", 106.35],
      ]),
      c("石嘴山", [
        ["市区", 106.39],
        ["大武口区", 106.37],
        ["惠农区", 106.79],
        ["平罗县", 106.54],
      ]),
      c("吴忠", [
        ["市区", 106.21],
        ["利通区", 106.22],
        ["红寺堡区", 106.07],
        ["盐池县", 107.41],
        ["同心县", 105.92],
        ["青铜峡市", 106.09],
      ]),
      c("固原", [
        ["市区", 106.25],
        ["原州区", 106.29],
        ["西吉县", 105.74],
        ["隆德县", 106.12],
        ["泾源县", 106.34],
        ["彭阳县", 106.64],
      ]),
      c("中卫", [
        ["市区", 105.2],
        ["沙坡头区", 105.2],
        ["中宁县", 105.69],
        ["海原县", 105.65],
      ]),
    ],
  },
  {
    name: "新疆",
    cities: [
      c("乌鲁木齐", [
        ["市区", 87.62],
        ["天山区", 87.64],
        ["沙依巴克区", 87.6],
        ["新市区", 87.58],
        ["水磨沟区", 87.65],
        ["头屯河区", 87.43],
        ["达坂城区", 88.32],
        ["米东区", 87.66],
        ["乌鲁木齐县", 87.42],
      ]),
      c("克拉玛依", [
        ["市区", 84.9],
        ["独山子区", 84.89],
        ["克拉玛依区", 84.87],
        ["白碱滩区", 85.14],
        ["乌尔禾区", 85.7],
      ]),
      c("吐鲁番", [
        ["市区", 89.2],
        ["高昌区", 89.19],
        ["鄯善县", 90.22],
        ["托克逊县", 88.66],
      ]),
      c("哈密", [
        ["市区", 93.52],
        ["伊州区", 93.52],
        ["巴里坤哈萨克自治县", 87.65],
        ["伊吾县", 94.7],
      ]),
      c("昌吉回族自治州", [
        ["市区", 87.32],
        ["昌吉市", 87.27],
        ["阜康市", 87.99],
        ["呼图壁县", 86.91],
        ["玛纳斯县", 86.22],
        ["奇台县", 89.6],
        ["吉木萨尔县", 89.19],
        ["木垒哈萨克自治县", 90.29],
      ]),
      c("博尔塔拉蒙古自治州", [
        ["市区", 82.07],
        ["博乐市", 82.06],
        ["阿拉山口市", 83.01],
        ["精河县", 82.9],
        ["温泉县", 81.03],
      ]),
      c("巴音郭楞蒙古自治州", [
        ["市区", 86.15],
        ["库尔勒市", 86.18],
        ["轮台县", 84.26],
        ["尉犁县", 86.27],
        ["若羌县", 88.18],
        ["且末县", 85.54],
        ["焉耆回族自治县", 86.58],
        ["和静县", 86.39],
        ["和硕县", 86.88],
        ["博湖县", 86.64],
        ["库尔勒经济技术开发区", 86.18],
      ]),
      c("阿克苏地区", [
        ["市区", 80.27],
        ["阿克苏市", 80.27],
        ["温宿县", 80.25],
        ["库车县", 82.97],
        ["沙雅县", 82.79],
        ["新和县", 82.62],
        ["拜城县", 81.86],
        ["乌什县", 79.23],
        ["阿瓦提县", 80.38],
        ["柯坪县", 79.05],
      ]),
      c("克孜勒苏柯尔克孜自治州", [
        ["市区", 76.17],
        ["阿图什市", 76.17],
        ["阿克陶县", 75.95],
        ["阿合奇县", 78.45],
        ["乌恰县", 75.27],
      ]),
      c("喀什地区", [
        ["市区", 76.0],
        ["喀什市", 76.0],
        ["疏附县", 75.87],
        ["疏勒县", 76.06],
        ["英吉沙县", 76.18],
        ["泽普县", 77.27],
        ["莎车县", 77.25],
        ["叶城县", 77.42],
        ["麦盖提县", 77.66],
        ["岳普湖县", 76.78],
        ["伽师县", 76.73],
        ["巴楚县", 78.56],
        ["塔什库尔干塔吉克自治县", 75.23],
      ]),
      c("和田地区", [
        ["市区", 79.93],
        ["和田市", 79.92],
        ["和田县", 79.92],
        ["墨玉县", 79.74],
        ["皮山县", 78.29],
        ["洛浦县", 80.2],
        ["策勒县", 80.81],
        ["于田县", 81.68],
        ["民丰县", 82.7],
      ]),
      c("伊犁哈萨克自治州", [
        ["市区", 81.33],
        ["伊宁市", 81.28],
        ["奎屯市", 84.91],
        ["霍尔果斯市", 80.43],
        ["伊宁县", 81.28],
        ["察布查尔锡伯自治县", 81.16],
        ["霍城县", 80.89],
        ["巩留县", 82.24],
        ["新源县", 83.27],
        ["昭苏县", 81.14],
        ["特克斯县", 81.84],
        ["尼勒克县", 82.52],
      ]),
      c("塔城地区", [
        ["市区", 82.99],
        ["塔城市", 82.99],
        ["乌苏市", 84.72],
        ["额敏县", 83.63],
        ["沙湾县", 85.63],
        ["托里县", 83.61],
        ["裕民县", 82.99],
        ["和布克赛尔蒙古自治县", 85.74],
      ]),
      c("阿勒泰地区", [
        ["市区", 88.15],
        ["阿勒泰市", 88.14],
        ["布尔津县", 86.88],
        ["富蕴县", 89.53],
        ["福海县", 87.49],
        ["哈巴河县", 86.42],
        ["青河县", 90.39],
        ["吉木乃县", 85.88],
      ]),
      c("自治区直辖县级行政区划", [["铁门关市", 106.58]]),
    ],
  },
  {
    name: "台湾",
    cities: [
      c("台北市", [
        ["中正区", 121.52],
        ["大同区", 121.51],
        ["中山区", 121.53],
        ["松山区", 121.56],
        ["大安区", 121.54],
        ["万华区", 121.5],
        ["信义区", 121.57],
        ["士林区", 121.52],
        ["北投区", 121.5],
        ["内湖区", 121.59],
        ["南港区", 121.61],
        ["文山区", 121.57],
      ]),
      c("新北市", [
        ["板桥区", 121.46],
        ["三重区", 121.49],
        ["中和区", 121.5],
        ["永和区", 121.52],
        ["新庄区", 121.45],
        ["新店区", 121.54],
        ["树林区", 121.42],
        ["莺歌区", 121.35],
        ["三峡区", 121.37],
        ["淡水区", 121.44],
        ["汐止区", 121.66],
        ["瑞芳区", 121.81],
        ["土城区", 121.44],
        ["芦洲区", 121.47],
        ["五股区", 121.44],
        ["泰山区", 121.43],
        ["林口区", 121.39],
        ["深坑区", 121.62],
        ["石碇区", 121.66],
        ["坪林区", 121.71],
        ["三芝区", 121.5],
        ["石门区", 121.57],
        ["八里区", 121.4],
        ["平溪区", 121.74],
        ["双溪区", 121.87],
        ["贡寮区", 121.92],
        ["金山区", 121.64],
        ["万里区", 121.69],
        ["乌来区", 121.55],
      ]),
      c("桃园市", [
        ["桃园区", 121.3],
        ["中坜区", 121.22],
        ["平镇区", 121.22],
        ["八德区", 121.28],
        ["杨梅区", 121.15],
        ["芦竹区", 121.29],
        ["大溪区", 121.29],
        ["龙潭区", 121.22],
        ["龟山区", 121.34],
        ["大园区", 121.2],
        ["观音区", 121.08],
        ["新屋区", 121.1],
        ["复兴区", 121.35],
      ]),
      c("台中市", [
        ["中区", 120.68],
        ["东区", 120.69],
        ["南区", 120.66],
        ["西区", 120.67],
        ["北区", 120.68],
        ["西屯区", 120.64],
        ["南屯区", 120.64],
        ["北屯区", 120.69],
        ["丰原区", 120.72],
        ["东势区", 120.83],
        ["大甲区", 120.62],
        ["清水区", 120.56],
        ["沙鹿区", 120.57],
        ["梧栖区", 120.53],
        ["后里区", 120.71],
        ["神冈区", 120.66],
        ["潭子区", 120.7],
        ["大雅区", 120.65],
        ["新社区", 120.81],
        ["石冈区", 120.78],
        ["外埔区", 120.65],
        ["大安区", 120.59],
        ["乌日区", 120.62],
        ["大肚区", 120.54],
        ["龙井区", 120.55],
        ["雾峰区", 120.7],
        ["太平区", 120.72],
        ["大里区", 120.68],
        ["和平区", 121.0],
      ]),
      c("台南市", [
        ["中西区", 120.2],
        ["东区", 120.22],
        ["南区", 120.19],
        ["北区", 120.21],
        ["安平区", 120.16],
        ["安南区", 120.18],
        ["永康区", 120.26],
        ["归仁区", 120.29],
        ["新化区", 120.31],
        ["左镇区", 120.41],
        ["玉井区", 120.46],
        ["楠西区", 120.49],
        ["南化区", 120.48],
        ["仁德区", 120.25],
        ["关庙区", 120.33],
        ["龙崎区", 120.37],
        ["官田区", 120.31],
        ["麻豆区", 120.25],
        ["佳里区", 120.18],
        ["西港区", 120.2],
        ["七股区", 120.14],
        ["将军区", 120.16],
        ["学甲区", 120.18],
        ["北门区", 120.13],
        ["新营区", 120.31],
        ["后壁区", 120.36],
        ["白河区", 120.42],
        ["东山区", 120.4],
        ["六甲区", 120.35],
        ["下营区", 120.26],
        ["柳营区", 120.31],
        ["盐水区", 120.27],
        ["善化区", 120.3],
        ["大内区", 120.35],
        ["山上区", 120.35],
        ["新市区", 120.29],
        ["安定区", 120.23],
      ]),
      c("高雄市", [
        ["楠梓区", 120.33],
        ["左营区", 120.29],
        ["鼓山区", 120.28],
        ["三民区", 120.31],
        ["盐埕区", 120.28],
        ["前金区", 120.29],
        ["新兴区", 120.3],
        ["苓雅区", 120.31],
        ["前镇区", 120.32],
        ["旗津区", 120.28],
        ["小港区", 120.34],
        ["凤山区", 120.36],
        ["大寮区", 120.4],
        ["鸟松区", 120.36],
        ["林园区", 120.4],
        ["仁武区", 120.35],
        ["大社区", 120.35],
        ["冈山区", 120.3],
        ["路竹区", 120.26],
        ["阿莲区", 120.33],
        ["田寮区", 120.36],
        ["燕巢区", 120.36],
        ["桥头区", 120.31],
        ["梓官区", 120.27],
        ["弥陀区", 120.25],
        ["永安区", 120.23],
        ["湖内区", 120.22],
        ["茄萣区", 120.18],
        ["旗山区", 120.48],
        ["美浓区", 120.54],
        ["六龟区", 120.63],
        ["甲仙区", 120.59],
        ["杉林区", 120.54],
        ["内门区", 120.46],
        ["茂林区", 120.66],
        ["桃源区", 120.76],
        ["那玛夏区", 120.7],
      ]),
      c("基隆市", [
        ["仁爱区", 121.74],
        ["信义区", 121.76],
        ["中正区", 121.77],
        ["中山区", 121.73],
        ["安乐区", 121.72],
        ["暖暖区", 121.74],
        ["七堵区", 121.71],
      ]),
      c("新竹市", [
        ["东区", 120.97],
        ["北区", 120.96],
        ["香山区", 120.92],
      ]),
      c("嘉义市", [
        ["东区", 120.46],
        ["西区", 120.43],
      ]),
      c("新竹县", [
        ["竹北市", 121.0],
        ["竹东镇", 121.09],
        ["新埔镇", 121.07],
        ["关西镇", 121.18],
        ["湖口乡", 121.04],
        ["新丰乡", 120.98],
        ["芎林乡", 121.08],
        ["横山乡", 121.12],
        ["北埔乡", 121.06],
        ["宝山乡", 120.99],
        ["峨眉乡", 121.01],
        ["尖石乡", 121.2],
        ["五峰乡", 121.1],
      ]),
      c("苗栗县", [
        ["苗栗市", 120.82],
        ["头份市", 120.9],
        ["竹南镇", 120.87],
        ["后龙镇", 120.78],
        ["通霄镇", 120.68],
        ["苑里镇", 120.65],
        ["卓兰镇", 120.82],
        ["大湖乡", 120.86],
        ["公馆乡", 120.82],
        ["铜锣乡", 120.78],
        ["南庄乡", 121.0],
        ["头屋乡", 120.85],
        ["三义乡", 120.74],
        ["西湖乡", 120.72],
        ["造桥乡", 120.86],
        ["三湾乡", 120.94],
        ["狮潭乡", 120.9],
        ["泰安乡", 120.9],
      ]),
      c("彰化县", [
        ["彰化市", 120.54],
        ["员林市", 120.57],
        ["和美镇", 120.5],
        ["鹿港镇", 120.43],
        ["溪湖镇", 120.48],
        ["二林镇", 120.37],
        ["田中镇", 120.58],
        ["北斗镇", 120.52],
        ["花坛乡", 120.53],
        ["芬园乡", 120.63],
        ["大村乡", 120.55],
        ["永靖乡", 120.55],
        ["伸港乡", 120.48],
        ["线西乡", 120.46],
        ["福兴乡", 120.44],
        ["秀水乡", 120.5],
        ["埔心乡", 120.54],
        ["埔盐乡", 120.46],
        ["大城乡", 120.32],
        ["芳苑乡", 120.32],
        ["竹塘乡", 120.42],
        ["社头乡", 120.58],
        ["二水乡", 120.62],
        ["田尾乡", 120.52],
        ["埤头乡", 120.46],
        ["溪州乡", 120.5],
      ]),
      c("南投县", [
        ["南投市", 120.69],
        ["草屯镇", 120.68],
        ["埔里镇", 120.97],
        ["竹山镇", 120.68],
        ["集集镇", 120.78],
        ["名间乡", 120.7],
        ["鹿谷乡", 120.75],
        ["中寮乡", 120.77],
        ["鱼池乡", 120.93],
        ["国姓乡", 120.86],
        ["水里乡", 120.86],
        ["信义乡", 120.85],
        ["仁爱乡", 121.13],
      ]),
      c("云林县", [
        ["斗六市", 120.54],
        ["斗南镇", 120.48],
        ["虎尾镇", 120.44],
        ["西螺镇", 120.46],
        ["土库镇", 120.39],
        ["北港镇", 120.3],
        ["古坑乡", 120.56],
        ["大埤乡", 120.43],
        ["莿桐乡", 120.51],
        ["林内乡", 120.61],
        ["二仑乡", 120.41],
        ["仑背乡", 120.35],
        ["麦寮乡", 120.25],
        ["东势乡", 120.25],
        ["褒忠乡", 120.31],
        ["台西乡", 120.2],
        ["元长乡", 120.32],
        ["四湖乡", 120.23],
        ["口湖乡", 120.18],
        ["水林乡", 120.24],
      ]),
      c("嘉义县", [
        ["太保市", 120.33],
        ["朴子市", 120.25],
        ["布袋镇", 120.17],
        ["大林镇", 120.47],
        ["民雄乡", 120.43],
        ["溪口乡", 120.4],
        ["新港乡", 120.35],
        ["六脚乡", 120.29],
        ["东石乡", 120.15],
        ["义竹乡", 120.24],
        ["鹿草乡", 120.31],
        ["水上乡", 120.4],
        ["中埔乡", 120.52],
        ["竹崎乡", 120.55],
        ["梅山乡", 120.55],
        ["番路乡", 120.59],
        ["大埔乡", 120.6],
        ["阿里山乡", 120.8],
      ]),
      c("屏东县", [
        ["屏东市", 120.49],
        ["潮州镇", 120.54],
        ["东港镇", 120.45],
        ["恒春镇", 120.74],
        ["万丹乡", 120.48],
        ["长治乡", 120.53],
        ["麟洛乡", 120.52],
        ["九如乡", 120.49],
        ["里港乡", 120.49],
        ["盐埔乡", 120.57],
        ["高树乡", 120.6],
        ["万峦乡", 120.57],
        ["内埔乡", 120.57],
        ["竹田乡", 120.54],
        ["新埤乡", 120.55],
        ["枋寮乡", 120.59],
        ["新园乡", 120.46],
        ["崁顶乡", 120.51],
        ["林边乡", 120.51],
        ["南州乡", 120.51],
        ["佳冬乡", 120.55],
        ["琉球乡", 120.37],
        ["车城乡", 120.71],
        ["满州乡", 120.84],
        ["枋山乡", 120.65],
        ["三地门乡", 120.65],
        ["雾台乡", 120.73],
        ["玛家乡", 120.64],
        ["泰武乡", 120.63],
        ["来义乡", 120.63],
        ["春日乡", 120.63],
        ["狮子乡", 120.7],
        ["牡丹乡", 120.78],
      ]),
      c("宜兰县", [
        ["宜兰市", 121.75],
        ["罗东镇", 121.77],
        ["苏澳镇", 121.84],
        ["头城镇", 121.82],
        ["礁溪乡", 121.77],
        ["壮围乡", 121.78],
        ["员山乡", 121.72],
        ["冬山乡", 121.79],
        ["五结乡", 121.8],
        ["三星乡", 121.65],
        ["大同乡", 121.5],
        ["南澳乡", 121.62],
      ]),
      c("花莲县", [
        ["花莲市", 121.61],
        ["凤林镇", 121.45],
        ["玉里镇", 121.31],
        ["新城乡", 121.64],
        ["吉安乡", 121.57],
        ["寿丰乡", 121.51],
        ["光复乡", 121.42],
        ["丰滨乡", 121.51],
        ["瑞穗乡", 121.38],
        ["富里乡", 121.25],
        ["秀林乡", 121.62],
        ["万荣乡", 121.41],
        ["卓溪乡", 121.3],
      ]),
      c("台东县", [
        ["台东市", 121.14],
        ["成功镇", 121.38],
        ["关山镇", 121.16],
        ["卑南乡", 121.08],
        ["大武乡", 120.9],
        ["太麻里乡", 121.01],
        ["东河乡", 121.3],
        ["长滨乡", 121.45],
        ["鹿野乡", 121.13],
        ["池上乡", 121.22],
        ["绿岛乡", 121.49],
        ["延平乡", 121.08],
        ["海端乡", 121.17],
        ["达仁乡", 120.88],
        ["金峰乡", 120.97],
        ["兰屿乡", 121.53],
      ]),
      c("澎湖县", [
        ["马公市", 119.57],
        ["湖西乡", 119.65],
        ["白沙乡", 119.6],
        ["西屿乡", 119.51],
        ["望安乡", 119.5],
        ["七美乡", 119.42],
      ]),
      c("金门县", [
        ["金城镇", 118.32],
        ["金湖镇", 118.42],
        ["金沙镇", 118.43],
        ["金宁乡", 118.31],
        ["烈屿乡", 118.24],
        ["乌坵乡", 119.45],
      ]),
      c("连江县", [
        ["南竿乡", 119.94],
        ["北竿乡", 120.0],
        ["莒光乡", 119.94],
        ["东引乡", 120.49],
      ]),
    ],
  },
  {
    name: "香港",
    cities: [
      c("香港岛", [
        ["中西区", 114.15],
        ["湾仔区", 114.18],
        ["东区", 114.22],
        ["南区", 114.17],
      ]),
      c("九龙", [
        ["油尖旺区", 114.17],
        ["深水埗区", 114.16],
        ["九龙城区", 114.19],
        ["黄大仙区", 114.2],
        ["观塘区", 114.22],
      ]),
      c("新界", [
        ["葵青区", 114.13],
        ["荃湾区", 114.12],
        ["屯门区", 113.97],
        ["元朗区", 114.03],
        ["北区", 114.15],
        ["大埔区", 114.17],
        ["沙田区", 114.19],
        ["西贡区", 114.27],
        ["离岛区", 113.95],
      ]),
    ],
  },
  {
    name: "澳门",
    cities: [
      c("澳门半岛", [
        ["花地玛堂区", 113.55],
        ["圣安多尼堂区", 113.54],
        ["大堂区", 113.54],
        ["望德堂区", 113.55],
        ["风顺堂区", 113.53],
      ]),
      c("氹仔", [["嘉模堂区", 113.56]]),
      c("路环", [
        ["圣方济各堂区", 113.56],
        ["路氹城", 113.57],
      ]),
    ],
  },
];

export function findLongitude(
  provinceName: string,
  cityName: string,
  districtName: string,
): number | undefined {
  const province = PROVINCES.find(p => p.name === provinceName);
  const city = province?.cities.find(ct => ct.name === cityName);
  const district = city?.districts.find(d => d.name === districtName);
  return district?.longitude;
}

export function getCityByName(
  name: string,
): { name: string; province: string; longitude: number } | undefined {
  for (const p of PROVINCES) {
    for (const ct of p.cities) {
      if (ct.name === name) {
        return { name: ct.name, province: p.name, longitude: ct.districts[0].longitude };
      }
    }
  }
  return undefined;
}

/* ── 自动生成的扁平列表与辅助函数 ─────────────────────── */

/** 所有省份名称 */
export const ALL_PROVINCE_NAMES: string[] = PROVINCES.map(p => p.name);

/** 所有城市名（去重） */
export const ALL_CITY_NAMES: string[] = [
  ...new Set(PROVINCES.flatMap(p => p.cities.map(c => c.name))),
];

/** 所有区县名（去重） */
export const ALL_DISTRICT_NAMES: string[] = [
  ...new Set(PROVINCES.flatMap(p => p.cities.flatMap(c => c.districts.map(d => d.name)))),
];

/** 扁平化城市经度列表（兼容旧接口） */
export const CITY_LNG: [string, number][] = PROVINCES.flatMap(p =>
  p.cities.map(ct => [ct.name, ct.districts[0].longitude] as [string, number]),
);

/** 获取某省的所有城市名 */
export function getCityNamesOfProvince(provinceName: string): string[] {
  return PROVINCES.find(p => p.name === provinceName)?.cities.map(c => c.name) ?? [];
}

/** 获取某省某市的所有区县名 */
export function getDistrictNamesOfCity(provinceName: string, cityName: string): string[] {
  const prov = PROVINCES.find(p => p.name === provinceName);
  return prov?.cities.find(c => c.name === cityName)?.districts.map(d => d.name) ?? [];
}

/** 通过省/市/区三级名称查经度 */
export function getLongitude(
  provinceName: string,
  cityName: string,
  districtName: string,
): number | undefined {
  const prov = PROVINCES.find(p => p.name === provinceName);
  const city = prov?.cities.find(c => c.name === cityName);
  const dist = city?.districts.find(d => d.name === districtName);
  return dist?.longitude;
}
