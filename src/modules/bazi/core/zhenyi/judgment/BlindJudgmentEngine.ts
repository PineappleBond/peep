// @ts-nocheck
// ---------------------------------------------------------------------------
// NOTE: ts-nocheck is applied because this engine was ported from an external
// JavaScript codebase (真一盲派) and its internal data structures are deeply
// intertwined with the untyped LocalPaipan output (Record<string, unknown>).
// Full type coverage requires first defining a typed schema for LocalPaipan's
// compute() result. Until then, the public entry point (computeBlindJudgment)
// returns a typed BlindJudgmentResult to callers.
// TODO: 当 LocalPaipan 完成类型化后，移除 @ts-nocheck 并为内部方法添加类型注解。
// ---------------------------------------------------------------------------
...
import {
  STEM_ELEMENT,
  BRANCH_ELEMENT,
  WU_XING_SHENG as ELEMENT_GENERATES,
  WU_XING_KE as ELEMENT_CONTROLS,
...
// STEM_ELEMENT, BRANCH_ELEMENT, ELEMENT_GENERATES, ELEMENT_CONTROLS 从 domainConstants 导入
...
export class BlindJudgmentEngine {
  ...
  analyze(chart:any, options:any = {}):BlindJudgmentResult {
    ...
    for (const path of validated) path.gong_level = this.resolveGongLevel(path, qishi);
  ...
  buildRelations(facts) {
    ...
    for (let i = 0; i < branches.length; i++) for (let j = i + 1; j < branches.length; j++) {
      ...
      if (BRANCH_HARMS.has(k)) add("harm", [a.id, b.id], { distance, adjacent: distance <= 1, school_label: "穿害" });
      ...
      if (distance <= 1 && elementGenerates(a.element, b.element)) add("generate", [a.id, b.id], { direction: `${a.id}>${b.id}`, distance, adjacent: true, medium: "branch" });
      if (distance <= 1 && elementGenerates(b.element, a.element)) add("generate", [b.id, a.id], { direction: `${b.id}>${a.id}`, distance, adjacent: true, medium: "branch" });
      if (distance <= 1 && elementControls(a.element, b.element)) add("control", [a.id, b.id], { direction: `${a.id}>${b.id}`, distance, adjacent: true, medium: "branch" });
      if (distance <= 1 && elementControls(b.element, a.element)) add("control", [b.id, a.id], { direction: `${b.id}>${a.id}`, distance, adjacent: true, medium: "branch" });
    }
    ...
    for (const pillar of PILLARS) {
      ...
      if (elementGenerates(br.element, st.element)) add("generate", [br.id, st.id], { direction: `${br.id}>${st.id}`, distance: 0, adjacent: true, medium: "same_pillar" });
      ...
      if (STABLE_SELF_COMBINE.has(pillarText)) add("stem_branch_combine", [st.id, br.id], { source: "school_whitelist", school: "真一盲派", distance: 0, adjacent: true });
    }
  ...
  resolveRelationSemantics(facts, relations) {
    ...
    for (const r of relations) {
      ...
      if (["stem_combine", "branch_combine", "stem_branch_combine"].includes(r.type)) {
        ...
        push(r, semantic, r.adjacent || r.distance === 0 ? "passed" : "conditional",
          [dayActor ? "日主直接参与合" : "原局存在真实合关系", r.adjacent ? "关系贴身或相邻" : "关系隔位，语义降级"],
          r.adjacent ? [] : ["隔位合不直接升级为取得/合去"], "REL-COMBINE-001");
  ...
  resolveGuestHost(facts, relations, context) {
    ...
    for (const r of (relations || [])) {
      if (!["sanhe", "sanhui"].includes(r.type)) continue;
      if (!(r.nodes || []).includes(facts.dayBranchNodeId)) continue;
    ...
    for (const n of facts.nodes) {
      ...
      if (n.position === "hidden_stem") weight = Math.max(0, weight - 0.05);
      const side = weight >= 0.65 ? "host" : weight <= 0.40 ? "guest" : "mixed";
      if (context === "family" && ["year", "month"].includes(n.pillar)) reason += " · 家庭主题下提高亲缘权重";
      if (context === "timing") reason += " · 岁运主题下原局整体作为主参照";
  ...
  resolveIntents(facts, relations, _guestHost) {
  ...
  resolveQiShi(facts, relations, _roots) {
  ...
  enumerateGongPaths(facts, relations, semantics, roots, guestHost, intents, qishi) {
    ...
    const classifyZhiTitle = (actor, target) => {
      ...
      if (actor.id !== facts.dayMasterNodeId && gh(actor.id) >= 0.65 && BODY_GODS.has(actor.tenGod) && USE_GODS.has(target.tenGod)) return `${actor.tenGod || "主位之体"}制${target.tenGod || "宾位之用"}`;
      return "";
    };
    ...
    if (qishi?.dominant?.targetElement && (qishi.dominant.elements || []).length) {
      ...
      for (const target of targets) {
        const connector = relations.filter(r => {
          ...
          if (!["clash", "control", "stem_branch_combine", "branch_combine", "harm", "punish"].includes(r.type)) return false;
          const members = (r.nodes || []).filter(id => memberIds.has(id) && id !== target.id);
          if (!members.length) return false;
          // 有方向的普通克制必须真的是“党势成员 → 目标”，不能把目标反过来克成员也算成党势制用。
          if (r.type === "control" && r.direction) return members.some(id => r.direction === `${id}>${target.id}`);
          return true;
        });
        ...
        const leadActors = uniq(connector.flatMap(r => {
          if (r.type === "control" && r.direction) return (r.nodes || []).filter(id => memberIds.has(id) && r.direction === `${id}>${target.id}`);
    ...
    for (const r of relations.filter(x => ["stem_combine", "branch_combine", "stem_branch_combine"].includes(x.type))) {
      ...
      for (const actor of ns) {
        ...
        if (!target) continue;
        if (gh(actor.id) < 0.65) continue;
        if (!(WEALTH_GODS.has(target.tenGod) || OFFICIAL_GODS.has(target.tenGod) || USE_GODS.has(target.tenGod))) continue;
  ...
  validatePath(path, facts, relations, roots, guestHost, _intents, _qishi) {
    ...
    if (actor?.position === "stem" && actorRoot?.actor_capable !== true && gh(actor.id) < 0.8 && path.type !== "he_yong") status = "intent_only";
  ...
  resolveOwnership(path, facts, relations, guestHost, origins) {
    ...
    let result = "unclear";
    const reasons = [];
    if (allResultHost) { result = (path.type === "hua_yong" || path.type === "he_yong") ? "mostly_native" : (anyActorGuest ? "shared" : "mostly_native"); reasons.push("结果节点落在日时主位"); }
    else if (hasDirectHostTake) { result = anyActorGuest ? "shared" : "mostly_native"; reasons.push(path.type === "he_yong" ? "主位直接合取目标" : "主位直接控制目标"); }
    else if (anyResultHost) { result = "shared"; reasons.push("结果同时连接主位与宾位"); }
    else if (resultWeights.length && resultWeights.every(x => x <= 0.4)) { result = "mostly_external"; reasons.push("结果节点主要停留在年月宾位"); }
    ...
    if (path.type === "sheng_yong" && result === "mostly_native" && resultWeights.some(x => x < 0.65)) result = "shared";
  ...
  resolvePathZhengFan(path, facts, intents, qishi, _guestHost) {
  ...
  resolveMainline(ranked, _facts, _intents, _qishi) {
  ...
  private resolveTombStates(facts:any,luckChars:any[],relevantIds:Set<string>):any[] {
    const out:any[]=[];const branches=(facts.branches||[]).map((id:string)=>facts.byId[id]).filter(Boolean);
    const chars=branches.map((n:any)=>n.char),has=(c:string)=>chars.includes(c);
    const add=(row:any)=>{const key=`${row.layer}|${row.inmateNodeId||''}|${row.storeNodeId||''}|${row.incoming_branch||''}|${row.action||''}`;if(out.some(x=>x._key===key))return;out.push({_key:key,id:`TS${String(out.length+1).padStart(3,'0')}`,...row});};
    for(const inmate of branches){
      const store=DIRECT_TOMB_STORE[inmate.char];if(!store)continue;
      const stores=branches.filter((b:any)=>b.char===store);
    ...
    for(const luck of luckChars){
      if(!luck.branch)continue;const lb=luck.branch;
      for(const inmate of branches){
        const store=DIRECT_TOMB_STORE[inmate.char];if(!store)continue;
  ...
  private resolveTombStateSnapshot(facts:any,luckChars:any[],relevantIds:Set<string>):any {
    const original=(facts.branches||[]).map((id:string)=>facts.byId[id]).filter(Boolean).map((n:any)=>({id:n.id,char:n.char,layer:'original',relevant:relevantIds.has(n.id)}));
    const active=[...original];
    for(const luck of luckChars){if(luck?.branch)active.push({id:`luck.${luck.layer}.branch`,char:luck.branch,layer:luck.layer,relevant:false});}
    const chars=active.map((x:any)=>x.char),has=(c:string)=>chars.includes(c);
    const states:any[]=[];
    const seen=new Set<string>();
    for(const inmate of active){
      const store=DIRECT_TOMB_STORE[inmate.char];if(!store)continue;
      for(const storeNode of active.filter((x:any)=>x.char===store&&x.id!==inmate.id)){
    ...
    for(const luck of luckChars){
      ...
      for(const storeNode of original.filter((x:any)=>Object.values(DIRECT_TOMB_STORE).includes(x.char))){
  ...
  private resolveCompositeLuckState(facts:any,luckChars:any[],relevantIds:Set<string>):any {
    const original=(facts.branches||[]).map((id:string)=>facts.byId[id]).filter(Boolean).map((n:any)=>({id:n.id,char:n.char,layer:'original'}));
    const stages=['dayun','liunian','liuyue'],layerRank=(l:string)=>l==='original'?-1:stages.indexOf(l);
    const snapshots:any[]=[];
    for(let si=0;si<stages.length;si++){
      const stage=stages[si];
      const active=[...original];
      for(const l of luckChars||[]){if(l?.branch&&stages.indexOf(l.layer)<=si)active.push({id:`luck.${l.layer}.branch`,char:l.branch,layer:l.layer});}
      const chars=active.map(x=>x.char);
      const formations:any[]=[];
      const check=(kind:string,groups:any[])=>{for(const g of groups){if(g.branches.every((z:string)=>chars.includes(z))){const members=g.branches.map((z:string)=>active.find(x=>x.char===z)).filter(Boolean);const firstRank=Math.max(...members.map((m:any)=>layerRank(m.layer)));const firstStage=firstRank<0?'original':stages[firstRank];formations.push({kind,branches:g.branches,result_element:g.element||null,family:g.family||null,formed:true,first_formed_stage:firstStage,newly_formed:firstStage===stage,relevant:members.some((m:any)=>m.layer==='original'&&relevantIds.has(m.id)),member_layers:members.map((m:any)=>({char:m.char,layer:m.layer})),transformation:'not_auto_assumed',source_rule:kind==='sanxing'?'BLIND-SANXING-COMPLETE-001':'BLIND-COMPOSITE-FORMATION-001'});}}};
  ...
  private replayMainlineState(primary:any,facts:any,relations:any[],luckRelations:any[],timingSemantics:any[],identityResolutions:any[],tombReplay:any,rootState:any,compositeState:any):any {
    ...
    const roleOf=(id:string)=>primary.actorNodes?.includes(id)?'actor':primary.targetNodes?.includes(id)?'target':primary.bridgeNodes?.includes(id)?'bridge':primary.resultNodes?.includes(id)?'result':'context';
    ...
    for(let si=0;si<stages.length;si++){
      ...
      const ids=(identityResolutions||[]).filter((x:any)=>stageIndex(x.layer)<=si).flatMap((x:any)=>(x.selected||[]).filter((c:any)=>c.evidence_grade==='A'&&relevantIds.has(c.targetNodeId)));
      ...
      for(const a of stageTomb.store_actions||[])if(a.relevant)changes.push({type:'tomb_action_candidate',layer:a.layer,role:'structure',certainty:'candidate',detail:a.detail,source_rule:a.source_rule});
  ...
  resolveTiming(chart, facts, relations, primary, options:any = {}) {
    ...
    const branchRel=(a:string,b:string)=>{
      ...
      const k=pairKey(a,b);
      if(BRANCH_COMBINES.has(k))return 'combine';if(BRANCH_CLASHES.has(k))return 'clash';if(BRANCH_HARMS.has(k))return 'harm';if(BRANCH_BREAKS.has(k))return 'break';if(PUNISH_PAIRS.has(k)||(a===b&&SELF_PUNISH.has(a)))return 'punish';return '';
    };
    ...
    const addLuck=(luck:any,n:any,type:string,detail:string,extra:any={})=>{const id=`LR${String(luckRelations.length+1).padStart(3,'0')}`;luckRelations.push({id,layer:luck.layer,type,targetNodeId:n.id,targetChar:n.char,detail,...extra});return id;};
    ...
    for(const ir of identityResolutions){
      ...
      for(const c of ir.selected.filter((x:any)=>x.major_eligible)){
    ...
    for(const t of triggers){
      if(t.layer==='liuyue'){t.major_eligible=false;t.scope='localization_only';continue;}
  ...
  buildEvidenceTrace(facts, relations, semantics, roots, intents, qishi, ranked, mainline, timing) {
    ...
    if (qishi.dominant) push("qishi", "气势辅助", [`${qishi.dominant.elements.join("、")}为当前加权优势气势`, `目标五行：${qishi.dominant.targetElement || "—"}`], ["QISHI-001"]);
    if (timing.available) {
  ...
  buildPresentation(result, _chart) {
  ...
  pathSummary(path, actor, target, bridge, _final) {
    ...
    if (path.type === "mu_yong") return `${simpleNodeLabel(target)}与对应墓库形成候选关系，但“入墓、开闭、归属”仍按条件式处理，不直接等同得失。`;
