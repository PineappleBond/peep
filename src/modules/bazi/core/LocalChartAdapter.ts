...
import {TrueSolarTimeCalculator} from './TrueSolarTimeCalculator';
import {WuXingScorer} from './WuXingScorer';
import {ShenShaMatcher} from './ShenShaMatcher';
import {
  STEM_ELEMENT, STEM_YIN_YANG, HIDDEN_STEMS, TWELVE_STAGES,
  NAYIN, SHI_SHEN_FULL,
} from './domainConstants';

export class LocalChartAdapter {
  private paipan=new LocalPaipan();
  compute(profile:any):any{
    ...
    const useTrueSolarTime=this.useTrueSolarTime(profile),longitude=TrueSolarTimeCalculator.resolveBirthLongitude(profile);
    if(useTrueSolarTime&&longitude===null)throw new Error('启用真太阳时时必须填写出生地点或经纬度');
    const timeZoneId=String(profile?.birth_timezone_id??'').trim(),timeZoneOffsetRaw=profile?.birth_timezone_offset;
    const timeBasis:any=timeZoneId||(timeZoneOffsetRaw!==undefined&&timeZoneOffsetRaw!==null&&timeZoneOffsetRaw!==''?Number(timeZoneOffsetRaw):8);
    const solarResult=longitude!==null?TrueSolarTimeCalculator.calculate(solarBirthday,birthTime,longitude,timeBasis):null;
    ...
    const info=this.paipan.GetInfo(gender,yy,mm,dd,hh,mt,0);if(!info||!Object.keys(info).length)throw new Error('本地排盘计算失败: GetInfo返回空');
    return this.formatResult(info,profile,solarResult,effectiveResult,gender,solarBirthday,useTrueSolarTime);
  }
  computeFromPillars(profile:any):any{
    ...
    const tg:number[]=[],dz:number[]=[];
    for(let i=0;i<4;i++){
      const chars=Array.from(pillars[i]);
      if(chars.length!==2)throw new Error(`${labels[i]}格式无效，请从六十甲子中选择`);
      const gi=this.paipan.ctg.indexOf(chars[0]),zi=this.paipan.cdz.indexOf(chars[1]);
      if(gi<0||zi<0||!NAYIN[pillars[i]])throw new Error(`${labels[i]}“${pillars[i]}”不是合法六十甲子`);
      tg.push(gi);dz.push(zi);
    }
    const tigerStart=[2,4,6,8,0][tg[0]%5],monthOffset=(dz[1]-2+12)%12,expectedMonth=(tigerStart+monthOffset)%10;
    if(tg[1]!==expectedMonth)throw new Error(`月柱与年柱不符合五虎遁规则：${pillars[0]}年对应当前月支应为${this.paipan.ctg[expectedMonth]}${this.paipan.cdz[dz[1]]}`);
    const ratStart=[0,2,4,6,8][tg[2]%5],expectedHour=(ratStart+dz[3])%10,nightZiHour=(expectedHour+2)%10,isZi=dz[3]===0;
    if(tg[3]!==expectedHour&&!(isZi&&tg[3]===nightZiHour))throw new Error(`时柱与日柱不符合五鼠遁规则：${pillars[2]}日对应当前时支应为${this.paipan.ctg[expectedHour]}${this.paipan.cdz[dz[3]]}${isZi?`，夜子时亦可为${this.paipan.ctg[nightZiHour]}子`:''}`);
    ...
    if(birthYearRaw!==''&&(!Number.isInteger(birthYear)||birthYear<1600||birthYear>2200))throw new Error('出生年份应填写 1600—2200 之间的完整年份');
    if(birthYear){const yg=((birthYear-4)%10+10)%10,yz=((birthYear-4)%12+12)%12;if(yg!==tg[0]||yz!==dz[0])throw new Error(`出生年份 ${birthYear} 与年柱 ${pillars[0]} 不一致，请检查输入`);}
    if(qyProvided&&(!Number.isInteger(qyYear)||qyYear<1||qyYear>10||!Number.isInteger(qyMonth)||qyMonth<0||qyMonth>11))throw new Error('盲派起运年龄应填写 1—10 虚岁；月份仅作手动补充');
    ...
    if(isZi)info.zi_hour_variant=tg[3]===nightZiHour?'night':'early';
  ...
  private buildDirectInfo(tg:number[],dz:number[],gender:number,birthYear:number,qyYear:number,qyMonth:number,qyProvided:boolean):any{
    const info:any={sex:gender,tg:[...tg],dz:[...dz],bazi:[],dz_cg:[],na_yin:[],direct_mode:true,manual_qiyun_provided:qyProvided,manual_start_age:qyYear,manual_qiyun_month:qyMonth,manual_start_year:(birthYear&&qyProvided)?birthYear+qyYear-1:0};
    for(let i=0;i<4;i++){info.bazi.push([this.paipan.ctg[tg[i]],this.paipan.cdz[dz[i]]]);const cg=this.paipan.dzcg[dz[i]]??[];info.dz_cg[i]={index:[...cg],char:cg.map(x=>this.paipan.ctg[x])};info.na_yin[i]=this.paipan.naYin(tg[i],dz[i]);}
    const yangYear=tg[0]%2===0,forward=(gender===0&&yangYear)||(gender===1&&!yangYear),big_tg:number[]=[],big_dz:number[]=[];
    for(let i=1;i<=12;i++){big_tg.push((tg[1]+(forward?i:20-i))%10);big_dz.push((dz[1]+(forward?i:24-i))%12);}
    info.big_tg=big_tg;info.big_dz=big_dz;info.big=big_tg.map((g,i)=>this.paipan.ctg[g]+this.paipan.cdz[big_dz[i]]);info.big_start_time=[];
    if(info.manual_start_year)for(let i=0;i<12;i++)info.big_start_time.push([info.manual_start_year+i*10,1,1,0,0,0]);
    const gd=(29-dz[1]-dz[3])%12,xi=gd<2?1:0,gt=((tg[0]%5)*2+gd+12*xi)%10;info.gong={index:[gt,gd],char:this.paipan.ctg[gt]+this.paipan.cdz[gd]};
    info.shen_gong=this.paipan.GetShenGong(tg[0],dz[1],dz[3]);info.tai_xi=this.paipan.GetTaiXi(tg[2],dz[2]);info.sx=this.paipan.csa[dz[0]];
    info.start_desc=qyProvided?`${qyYear}虚岁起运（四柱直排手动填写）`:'';info.start_time=info.manual_start_year?[info.manual_start_year,1,1,0,0,0]:[];
    const nayinElement=Number(info.na_yin?.[0]?.[1]);
    if(birthYear){
      info.jiaoyun_rule=this.buildDirectJiaoyunRule(nayinElement);info.jiaoyun_desc=info.jiaoyun_rule.desc;
      if(qyProvided&&info.manual_start_year){
        ...
        info.start_event=info.jiaoyun_events?.[0]??null;info.start_time=info.start_event?.event_start_parts??info.start_time;info.start_time_window_end=info.start_event?.event_end_parts??[];
        info.big_start_time=[];for(let i=0;i<12;i++)info.big_start_time.push(info.jiaoyun_events?.[i*2]?.event_start_parts??[info.manual_start_year+i*10,1,1,0,0,0]);
      }
    }
    else info.jiaoyun_desc=qyProvided?'四柱直排 · 已手动填写起运年龄；填写出生年份后可生成纳音交运规则':'';
    info.wx_fen=this.paipan.wuXingPingFen(info);return info;
  }
  private buildDirectJiaoyunRule(nayinElement:number):any{
    // 四柱直排与日期排盘必须共用同一条盲派交运规则，避免两套口径漂移。
    return this.paipan.getBlindJiaoyunRule(nayinElement);
  }
  private useTrueSolarTime(profile:any):boolean{const v=profile?.use_true_solar_time;return !(v===false||v===0||v==='0'||String(v).toLowerCase()==='false');}
  private buildCivilTimeResult(birthday:string,birthTime:string):any{
  ...
  private resolveSolarBirthday(profile:any):string{
    const birthday=String(profile.birthday??'').trim().split(' ')[0];if(!Number(profile.is_lunar??0))return birthday;
    const m=/^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(birthday);if(!m)throw new Error('出生日期格式无效: '+birthday);
    const solar=this.paipan.Lunar2Solar(+m[1],+m[2],+m[3],Number(profile.is_leap??0));if(!solar||solar.length<3)throw new Error(`农历转公历失败: ${+m[1]}-${+m[2]}-${+m[3]}${Number(profile.is_leap??0)?'（闰月）':''}`);
    return `${solar[0].toString().padStart(4,'0')}-${solar[1].toString().padStart(2,'0')}-${solar[2].toString().padStart(2,'0')}`;
  }
  private formatResult(info:any,profile:any,solarResult:any,effectiveResult:any,gender:number,solarBirthday:string,useTrueSolarTime:boolean):any{
    ...
    const hidden_stems:any={};for(const [pos,branch] of Object.entries({year:yb,month:mb,day:db,hour:hb}))hidden_stems[pos]=this.buildHiddenStemsForBranch(ds,String(branch));
    const elements:Record<string,number>={金:0,木:0,水:0,火:0,土:0};for(const st of [ys,ms,ds,hs])if(STEM_ELEMENT[st])elements[STEM_ELEMENT[st]]++;for(const br of [yb,mb,db,hb]){const st=HIDDEN_STEMS[br]?.[0];if(st&&STEM_ELEMENT[st])elements[STEM_ELEMENT[st]]++;}const missing=Object.entries(elements).filter(([,v])=>v===0).map(([k])=>k),summary=missing.length?'五行缺'+missing.join('、'):'五行俱全';
    ...
    let lunar:any={year:'',month:'',day:'',is_leap:false,year_text:'',month_text:'',day_text:'',text:'',hour:''};if(!isDirect){const solarParts=solarBirthday.split('-').map(Number),lunarRaw=this.paipan.Solar2Lunar(solarParts[0],solarParts[1],solarParts[2]);lunar=this.buildLunarInfo(lunarRaw,b[info.dz[3]]+'时');}
  ...
  private buildDaYun(info:any,dayStem:string,birthYear:number):any[]{
    const s=this.paipan.ctg,b=this.paipan.cdz,jy=Number(info.start_time?.[0]??0),direct=!!info.direct_mode,manual=!!info.manual_qiyun_provided,manualAge=Number(info.manual_start_age??0),manualStartYear=Number(info.manual_start_year??0),startAge=direct?(manual?manualAge:0):(Number(info.qiyun_virtual_age||0)|| (jy&&birthYear?jy-birthYear+1:0)),out:any[]=[];
    const events:any[]=info.jiaoyun_events??[],eventByYear=new Map(events.map((e:any)=>[Number(e.year),e]));
    ...
    for(let di=0;di<out.length;di++){
      const dy=out[di],list:any[]=[];
      if(dy.start_year)for(let j=0;j<10;j++){
  ...
  buildFlowMonths(flowYear:number,dayStem:string):any[]{
    const branches=['寅','卯','辰','巳','午','未','申','酉','戌','亥','子','丑'],terms=['立春','惊蛰','清明','立夏','芒种','小暑','立秋','白露','寒露','立冬','大雪','小寒'];
    const yearStemIndex=((flowYear-4)%10+10)%10,tigerStart=[2,4,6,8,0][yearStemIndex%5],out:any[]=[];
    for(let i=0;i<12;i++){
      const startYear=i===11?flowYear+1:flowYear,endYear=i>=10?flowYear+1:flowYear,startTerm=terms[i],endTerm=i===11?'立春':terms[i+1];
      const startEvent=this.buildTermEvent(startYear,startTerm),endEvent=this.buildTermEvent(endYear,endTerm);if(!startEvent||!endEvent)continue;
  ...
  private buildTermEvent(year:number,term:string):any{
    const parts=this.paipan.getSolarTermParts(year,term);if(!parts)return null;
    const fmt=(p:number[])=>`${String(p[0]).padStart(4,'0')}-${String(p[1]).padStart(2,'0')}-${String(p[2]).padStart(2,'0')} ${String(p[3]||0).padStart(2,'0')}:${String(p[4]||0).padStart(2,'0')}:${String(p[5]||0).padStart(2,'0')}`;
  ...
  private buildBlindDaXian(birthYear:number):any{
    if(!birthYear)return {available:false,rule_version:'blind-majority-35',ranges:[],note:'未提供出生年份，不生成当前大限年龄背景'};
  ...
  private buildXiaoYun(info:any,gender:number,count=100):string[]{const s=this.paipan.ctg,b=this.paipan.cdz,hg=info.tg?.[3]??0,hb=info.dz?.[3]??0,yg=info.tg?.[0]??0,yang=yg%2===0,forward=(gender===0&&yang)||(gender===1&&!yang),step=forward?1:-1,out:string[]=[];for(let i=1;i<=count;i++)out.push(s[((hg+step*i)%10+10)%10]+b[((hb+step*i)%12+12)%12]);return out;}
  private buildHiddenStemsForBranch(dayStem:string,branch:string):any[]{const labels=['本气','中气','余气'];return (HIDDEN_STEMS[branch]??[]).map((stem,i)=>({stem,label:labels[i]??'',element:STEM_ELEMENT[stem]??'',shi_shen:this.getShiShen(dayStem,stem)}));}
  private getShiShen(dayStem:string,otherStem:string):string{if(dayStem===otherStem)return '比肩';const a=this.paipan.ctg.indexOf(dayStem),b=this.paipan.ctg.indexOf(otherStem);if(a<0||b<0)return '';const r=this.paipan.GetTenGod(a,b);return SHI_SHEN_FULL[r.char]??r.char??'';}
  private calcKongWang(pillar:string):string{const s=this.paipan.ctg,b=this.paipan.cdz,c=Array.from(pillar);if(c.length<2)return '';const si=s.indexOf(c[0]),bi=b.indexOf(c[1]);if(si<0||bi<0)return '';const start=(bi-si+12)%12;return b[(start+10)%12]+b[(start+11)%12];}
  private calcTaiYuan(ms:string,mb:string):string{const s=this.paipan.ctg,b=this.paipan.cdz,si=s.indexOf(ms),bi=b.indexOf(mb);return si<0||bi<0?'':s[(si+1)%10]+b[(bi+3)%12];}
  private buildJieQiInfo(info:any):any{const names=['小寒','立春','惊蛰','清明','立夏','芒种','小暑','立秋','白露','寒露','立冬','大雪','小寒','立春','惊蛰','清明'],jq=info.jq_table??[],ix=info.jq_ix,jd=info.birth_jd;if(ix===null||ix===undefined||jd===null||jd===undefined||!jq.length||jq[ix]===undefined||jq[ix+1]===undefined)return {prev_jie:null,next_jie:null,jieqi_desc:''};const prev=jq[ix],next=jq[ix+1],pn=names[ix]??'',nn=names[ix+1]??'',pd=this.jdToDateString(prev),nd=this.jdToDateString(next),pdays=jd-prev,ndays=next-jd,pdi=Math.floor(pdays),phi=Math.floor((pdays-pdi)*24),ndi=Math.floor(ndays),nhi=Math.floor((ndays-ndi)*24);return {prev_jie:{name:pn,date:pd},next_jie:{name:nn,date:nd},jieqi_desc:`出生于${pn}后${pdi}天${phi}小时，${nn}前${ndi}天${nhi}小时`};}
  private jdToDateString(jd:number):string{const a=this.paipan.Julian2Solar(jd);if(!a||a.length<6)return '';return `${String(a[0]).padStart(4,'0')}-${String(a[1]).padStart(2,'0')}-${String(a[2]).padStart(2,'0')} ${String(a[3]).padStart(2,'0')}:${String(a[4]).padStart(2,'0')}:${String(a[5]).padStart(2,'0')}`;}
  computeShenSha(chartData:any,gender=0):any{const b=chartData.bazi??{},input:any={};for(const [cn,en] of Object.entries({'年柱':'year_pillar','月柱':'month_pillar','日柱':'day_pillar','时柱':'hour_pillar'})){const p=(b as any)[en as string]??{};input[cn]={gan:p.heavenly_stem??'',zhi:p.earthly_branch??''};}const matcher=new ShenShaMatcher(),main=matcher.matchAll(input,{} as Record<string, unknown>,{gender:String(gender)}),dayun:any[]=[],liunian:any[]=[];for(let i=0;i<(chartData.da_yun??[]).length;i++){const step=chartData.da_yun[i],c=Array.from(step.pillar??'');if(c.length<2)continue;const extra:any={'大运':{gan:c[0],zhi:c[1]}},matches=matcher.matchAll(input,extra,{gender:String(gender)}),names:string[]=[];for(const r of matches)for(const m of r.matches??[])if(m.target_pillar==='大运'&&r.name&&!names.includes(r.name))names.push(r.name);dayun.push([step.pillar,names]);const each:any={};for(const ln of step.liu_nian??[]){const lc=Array.from(ln.ganzhi??'');if(lc.length<2)continue;const ex={'大运':{gan:c[0],zhi:c[1]},'流年':{gan:lc[0],zhi:lc[1]}},lm=matcher.matchAll(input,ex,{gender:String(gender)}),nms:string[]=[];for(const r of lm)for(const m of r.matches??[])if(m.target_pillar==='流年'&&r.name&&!nms.includes(r.name))nms.push(r.name);each[Number(ln.year)]=[ln.ganzhi,nms];}liunian[i]=each;}return {main,dayun,liunian};}
}
