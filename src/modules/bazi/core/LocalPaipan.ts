...
export class LocalPaipan {
  ...
  private GetSMsinceWinterSolstice(yy:number,jdws:number):number[]{
    ...
    const [kn]=this.MeanNewMoon(jd); for(let i=0;i<=19;i++){const k=kn+i;tjd[i]=this.TrueNewMoon(k)+1/3-this.DeltaT(yy,i-1)/1440;}
  ...
  public Solar2Lunar(yy:number,mm:number,dd:number):number[]|false{
    if(!this.ValidDate(yy,mm,dd))return false;let prev=0,isLeap=0;let [,jdnm,mc]=this.GetZQandSMandLunarMonthCode(yy);const jd=this.Solar2Julian(yy,mm,dd,12,0,0);if(jd===false)return false;if(Math.floor(jd)<Math.floor(jdnm[0]+0.5)){prev=1;[,jdnm,mc]=this.GetZQandSMandLunarMonthCode(yy-1);}let mi=0;for(let i=0;i<=14;i++){if(Math.floor(jd)>=Math.floor(jdnm[i]+0.5)&&Math.floor(jd)<Math.floor(jdnm[i+1]+0.5)){mi=i;break;}}if(mc[mi]<2||prev===1)yy--;if((mc[mi]-Math.floor(mc[mi]))*2+1!==1)isLeap=1;mm=(Math.floor(mc[mi]+10)%12)+1;dd=Math.floor(jd)-Math.floor(jdnm[mi]+0.5)+1;return [yy,mm,dd,isLeap];
  }
